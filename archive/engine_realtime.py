...
class RealTimeSimulationEngine:
    def __init__(self, params, data_queue):
        ...
        self.running = False
        self.floaters = [
            Floater(i, params) for i in range(params.get("num_floaters", 1))
        ]
        self.time = 0.0
        ...
        # Pulse scheduling
        self.pulse_interval = params.get(
            "pulse_interval", 2.0
    ...
    def send_sse_data(self, data):
        ...
        for (
            client
        ) in self.sse_clients.copy():  # Copy to avoid modification during iteration
            try:
    ...
    def update_params(self, params):
        ...
        validated_params["num_floaters"] = max(
            1, min(100, params.get("num_floaters", 8))
        )
        validated_params["floater_volume"] = max(
            0.01, min(10.0, params.get("floater_volume", 0.3))
        )
        validated_params["floater_mass_empty"] = max(
            0.1, min(1000.0, params.get("floater_mass_empty", 18.0))
        )
        validated_params["floater_area"] = max(
            0.001, min(10.0, params.get("floater_area", 0.035))
        )
        validated_params["airPressure"] = max(
            0.1, min(50.0, params.get("airPressure", 3.0))
        )

        # Pulse physics parameters validation
        validated_params["air_fill_time"] = max(
            0.1, min(5.0, params.get("air_fill_time", 0.5))
        )
        validated_params["air_pressure"] = max(
            100000, min(1000000, params.get("air_pressure", 300000))
        )
        validated_params["air_flow_rate"] = max(
            0.1, min(10.0, params.get("air_flow_rate", 0.6))
        )
        validated_params["jet_efficiency"] = max(
            0.1, min(1.0, params.get("jet_efficiency", 0.85))
        )
        validated_params["sprocket_radius"] = max(
            0.1, min(2.0, params.get("sprocket_radius", 0.5))
        )
        validated_params["flywheel_inertia"] = max(
            1.0, min(1000.0, params.get("flywheel_inertia", 50.0))
        )
        validated_params["pulse_interval"] = max(
            0.5, min(10.0, params.get("pulse_interval", 2.0))
        )
        validated_params["load_torque"] = max(
            0.0, min(10000.0, params.get("load_torque", 100.0))
        )

        # H1/H2 effect parameters validation
        validated_params["nanobubble_frac"] = max(
            0.0, min(1.0, params.get("nanobubble_frac", 0.0))
        )
        validated_params["thermal_coeff"] = max(
            0.0, min(0.01, params.get("thermal_coeff", 0.0001))
        )
        validated_params["water_temp"] = max(
            -10.0, min(100.0, params.get("water_temp", 20.0))
        )
        validated_params["ref_temp"] = max(
            -10.0, min(100.0, params.get("ref_temp", 20.0))
        )
    ...
    def step(self, dt):
        ...
        pulse_forces = sum(
            floater.pulse_force for floater in self.floaters
        )  # Calculate torques using pulse physics
        base_torque = (
            sum(abs(floater.force) for floater in self.floaters)
            * self.pulse_physics.r_sprocket
        )
        ...
        if flywheel_speed < 1.0:
            ...
            speed_ratio = flywheel_speed / target_omega
            power_consumed = target_power * min(
                3.0, 1.5 * speed_ratio
            )  # Cap at 1.59MW max
        ...
        if flywheel_speed > 0.1:
            ...
            self.pulse_physics.omega_flywheel = max(
                0.0, self.pulse_physics.omega_flywheel
            )
        ...
        efficiency = (
            (power_output / theoretical_max_power * 100)
        ...
        # Average velocity
        velocity = (
            sum(floater.velocity for floater in self.floaters) / len(self.floaters)
            if self.floaters
            else 0
        )
