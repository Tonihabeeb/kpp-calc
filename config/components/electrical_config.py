...
class ElectricalConfig(BaseConfig):
    ...
    efficiency_at_rated: float = field(
        default=0.94, metadata={"description": "Efficiency at rated load", "min": 0, "max": 1}
    )

    # Power electronics properties
    rectifier_efficiency: float = field(
        default=0.97, metadata={"description": "Rectifier efficiency", "min": 0, "max": 1}
    )
    inverter_efficiency: float = field(
        default=0.96, metadata={"description": "Inverter efficiency", "min": 0, "max": 1}
    )
    transformer_efficiency: float = field(
        default=0.985, metadata={"description": "Transformer efficiency", "min": 0, "max": 1}
    )
    ...
    voltage_regulation_band: float = field(
        default=0.05, metadata={"description": "Voltage regulation band", "min": 0, "max": 1}
    )
    frequency_regulation_band: float = field(
        default=0.1, metadata={"description": "Frequency regulation band", "min": 0, "max": 1}
    )
