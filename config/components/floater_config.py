...
class FloaterConfig(BaseConfig):
    ...
    air_pressure: float = field(
        default=300000.0, metadata={"description": "Air pressure in Pa", "min": 50000, "max": 1000000}
    )
    ...
    heat_transfer_coefficient: float = field(
        default=150.0, metadata={"description": "Heat transfer coefficient in W/m²K", "min": 0, "max": 1000}
    )
    specific_heat_air: float = field(
        default=1005.0, metadata={"description": "Specific heat of air in J/kg·K", "min": 0}
    )
    specific_heat_water: float = field(
        default=4186.0, metadata={"description": "Specific heat of water in J/kg·K", "min": 0}
    )

    # State machine properties
    fill_state: str = field(default="empty", metadata={"description": "Initial fill state"})
    air_fill_level: float = field(
        default=0.0, metadata={"description": "Initial air fill level (0-1)", "min": 0, "max": 1}
    )
