...
from .core.schema import validate_config_against_schema
...
class ConfigManager:
    ...
    def load_config_from_file(self, config_name: str) -> bool:
        ...
        try:
            ...
            if 'drivetrain' in config_data:
                self.drivetrain_config = DrivetrainConfig(**config_data['drivetrain'])
    ...
    def save_config_to_file(self, config_name: str) -> bool:
        ...
        try:
            config_data = {
                'simulation': self.simulation_config.to_dict() if self.simulation_config else {},
                'floater': self.floater_config.to_dict() if self.floater_config else {},
                'electrical': self.electrical_config.to_dict() if self.electrical_config else {},
                'drivetrain': self.drivetrain_config.to_dict() if self.drivetrain_config else {},
                'control': self.control_config.to_dict() if self.control_config else {}
            }
    ...
    def validate_all_configs(self) -> bool:
        ...
        configs = [
            ...
            ('drivetrain', self.drivetrain_config),
    ...
    def get_combined_config(self) -> Dict[str, Any]:
        ...
        if self.simulation_config:
            combined.update(self.simulation_config.to_dict())
        
        if self.floater_config:
            combined.update(self.floater_config.to_dict())
        
        if self.electrical_config:
            combined.update(self.electrical_config.to_dict())
        
        if self.drivetrain_config:
            combined.update(self.drivetrain_config.to_dict())
        
        if self.control_config:
            combined.update(self.control_config.to_dict())
