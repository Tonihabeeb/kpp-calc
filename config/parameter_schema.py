...
PARAM_SCHEMA = {
    ...
    'floater_Cd': {'type': float, 'min': 0.1, 'max': 2.0, 'default': 0.8},
    ...
    'nanobubble_generation_power': {'type': float, 'min': 0.0, 'max': 10000.0, 'default': 2500.0},
    ...
    'water_temperature': {'type': float, 'min': 273.15, 'max': 373.15, 'default': 293.15},
    ...
    'max_simulation_time': {'type': float, 'min': 10.0, 'max': 3600.0, 'default': 300.0},
...
def validate_parameters_batch(params: Dict[str, Any]) -> Dict[str, Any]:
    ...
    logger.info(f"Parameter validation: {len(params)} params, "
...
def validate_parameters(params: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if not result['valid']:
        error_messages = [f"{err['parameter']}: {err['error']}" for err in result['errors']]
