            ...
            if EmergencyType.ELECTRICAL_FAULT not in self.active_emergencies:
                new_emergencies.append(
                    EmergencyCondition(
                        ...
                        detected_time=current_time,
                        threshold_value=(
                            self.limits.min_grid_voltage
                            if grid_voltage < self.limits.min_grid_voltage
                            else self.limits.max_grid_voltage
                        ),
            ...
            if EmergencyType.GRID_DISTURBANCE not in self.active_emergencies:
                new_emergencies.append(
                    EmergencyCondition(
                        ...
                        detected_time=current_time,
                        threshold_value=(
                            self.limits.min_grid_frequency
                            if grid_frequency < self.limits.min_grid_frequency
                            else self.limits.max_grid_frequency
                        ),
    ...
    def _update_emergency_response(self, system_state: Dict, current_time: float) -> Dict:
        ...
        commands = {
            ...
            "emergency_conditions": [
                ...
                for cond in self.emergency_conditions
                if not cond.resolved
            ],
        }
