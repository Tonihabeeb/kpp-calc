...
class GridStabilityController:
    ...
    def __init__(
        ...
        logger.info(
            f"GridStabilityController initialized: {rated_power/1000:.1f}kW, {nominal_voltage}V, {nominal_frequency}Hz"
        )
        ...
        for fault in self.active_faults:
            ...
            if fault.fault_type == "voltage":
                ...
                fault_cleared = (
                    ...
                    and self.voltage_thd <= self.limits.voltage_thd_max
                )
    ...
    def _update_performance_tracking(self, dt: float):
        ...
        if (
            self.overall_stability_index < 0.8
            or len(self.active_faults) > 0
            or self.current_mode != GridStabilityMode.NORMAL
        ):
