...
class GridDisturbanceHandler:
    ...
    def _detect_grid_disturbances(self, current_time: float) -> List[DisturbanceEvent]:
        ...
        if freq_error > self.limits.frequency_deadband:
            disturbance_type = (DisturbanceType.FREQUENCY_HIGH if self.grid_frequency > self.limits.nominal_frequency 
            ...
            if not any(d.disturbance_type == disturbance_type and not d.resolved for d in self.active_disturbances):
                new_disturbances.append(DisturbanceEvent(
        ...
        if voltage_error > self.limits.voltage_deadband:
            disturbance_type = (DisturbanceType.VOLTAGE_HIGH if self.grid_voltage > self.limits.nominal_voltage 
        ...
        if not self.grid_connected:
            if not any(d.disturbance_type == DisturbanceType.GRID_OUTAGE and not d.resolved for d in self.active_disturbances):
                new_disturbances.append(DisturbanceEvent(
        ...
        for disturbance in new_disturbances:
            ...
            logger.warning(f"Grid disturbance detected: {disturbance.disturbance_type.value} - magnitude: {disturbance.magnitude:.3f}")
    ...
    def _determine_response_mode(self) -> ResponseMode:
        ...
        for disturbance in self.active_disturbances:
            if not disturbance.resolved:
                ...
                if (disturbance.disturbance_type in [DisturbanceType.FREQUENCY_HIGH, DisturbanceType.FREQUENCY_LOW] and
                    disturbance.magnitude > self.limits.frequency_emergency):
                    return ResponseMode.DISCONNECT
                
                if (disturbance.disturbance_type in [DisturbanceType.VOLTAGE_HIGH, DisturbanceType.VOLTAGE_LOW] and
                    disturbance.magnitude > self.limits.voltage_emergency):
                    return ResponseMode.DISCONNECT
        ...
        for disturbance in self.active_disturbances:
            if not disturbance.resolved:
                if (disturbance.disturbance_type in [DisturbanceType.FREQUENCY_HIGH, DisturbanceType.FREQUENCY_LOW] and
                    disturbance.magnitude > self.limits.frequency_critical):
                    return ResponseMode.LOAD_SHEDDING
                
                if (disturbance.disturbance_type in [DisturbanceType.VOLTAGE_HIGH, DisturbanceType.VOLTAGE_LOW] and
                    disturbance.magnitude > self.limits.voltage_critical):
                    return ResponseMode.LOAD_SHEDDING
        ...
        for disturbance in self.active_disturbances:
            if not disturbance.resolved:
                if (disturbance.disturbance_type in [DisturbanceType.FREQUENCY_HIGH, DisturbanceType.FREQUENCY_LOW] and
                    disturbance.magnitude > self.limits.frequency_warning):
                    return ResponseMode.FREQUENCY_SUPPORT
                
                if (disturbance.disturbance_type in [DisturbanceType.VOLTAGE_HIGH, DisturbanceType.VOLTAGE_LOW] and
                    disturbance.magnitude > self.limits.voltage_warning):
    ...
    def _generate_frequency_support_commands(self, system_state: Dict, current_time: float) -> Dict:
        ...
        if abs(freq_error) > self.frequency_deadband:
            ...
            frequency_response = np.clip(frequency_response, -self.max_frequency_response, self.max_frequency_response)
    ...
    def _generate_load_shedding_commands(self, system_state: Dict, current_time: float) -> Dict:
        ...
        for disturbance in self.active_disturbances:
            if not disturbance.resolved:
                if disturbance.disturbance_type in [DisturbanceType.FREQUENCY_HIGH, DisturbanceType.FREQUENCY_LOW]:
                ...
                elif disturbance.disturbance_type in [DisturbanceType.VOLTAGE_HIGH, DisturbanceType.VOLTAGE_LOW]:
    ...
    def _update_disturbance_resolution(self, current_time: float):
        ...
        for disturbance in self.active_disturbances:
            ...
            if self._is_disturbance_resolved(disturbance):
                ...
                logger.info(f"Grid disturbance resolved: {disturbance.disturbance_type.value} after {disturbance.duration:.1f}s")
