...
class Drivetrain:
    ...
    def __init__(
        ...
        if gear_ratio <= 0 or not (0 <= efficiency <= 1) or sprocket_radius <= 0:
            logger.error(
                "Invalid drivetrain parameters: gear_ratio and sprocket_radius must be positive, efficiency in [0,1]."
            )
    ...
    def compute_input_torque(self, chain_force: float) -> float:
        ...
        logger.debug(
            f"Computed input torque: {input_torque:.2f} Nm (chain_force={chain_force:.2f})"
        )
        return input_torque
    ...
    def update_dynamics(self, net_torque: float, load_torque: float, dt: float):
        ...
        logger.debug(
            f"Clutch logic: speed_difference={speed_difference:.4f}, omega_chain={self.omega_chain:.4f}, net_torque={net_torque:.4f}, static_friction_threshold={static_friction_threshold}"
        )
        # Allow clutch engagement at zero speed if net torque is positive (creep mode)
        if (speed_difference < self.omega_threshold and self.omega_chain >= 0.0) or (
            self.omega_chain == 0.0 and net_torque > 0
        ):
            logger.debug(
                f"Clutch engaged: (speed_difference < {self.omega_threshold})={speed_difference < self.omega_threshold}, (omega_chain >= 0.0)={self.omega_chain >= 0.0}, (omega_chain == 0.0 and net_torque > 0)={self.omega_chain == 0.0 and net_torque > 0}"
            )
            ...
            logger.debug(
                f"Clutch disengaged: (speed_difference >= {self.omega_threshold})={speed_difference >= self.omega_threshold}, (omega_chain < 0.0)={self.omega_chain < 0.0}, (omega_chain == 0.0 and net_torque <= 0)={self.omega_chain == 0.0 and net_torque <= 0}"
            )
            self.clutch_engaged = False
            alpha_chain = (
                (net_torque - T_drag_chain) / self.I_chain if self.I_chain > 0 else 0
            )
            self.omega_chain += alpha_chain * dt

            alpha_flywheel = (
                -T_drag_flywheel / self.I_flywheel if self.I_flywheel > 0 else 0
            )
        ...
        logger.debug(
            f"Drivetrain updated: omega_chain={self.omega_chain:.2f}, omega_flywheel={self.omega_flywheel:.2f}, clutch={self.clutch_engaged}"
        )
