...
class GearStage:
    ...
    def update(self, input_speed: float, input_torque: float):
        ...
        logger.debug(
            f"GearStage - Ratio: {self.ratio:.2f}, "
...
class Gearbox:
    ...
    def update(self, input_speed: float, input_torque: float):
        ...
        logger.debug(
            f"Gearbox - Input: {self.input_shaft_speed:.2f} rad/s, {self.input_torque:.2f} N·m, "
...
def create_kpp_gearbox() -> Gearbox:
    ...
    logger.info(
        f"Created KPP gearbox with overall ratio: {gearbox.overall_ratio:.1f}:1, "
