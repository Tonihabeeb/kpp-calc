...
class FloaterStateMachine:
    ...
    def update(self, context: dict) -> FloaterState:
        ...
        for transition in self.transitions:
            if transition.from_state == self.current_state and transition.condition(context):
                ...
                self.state_history.append(
                    {"from_state": old_state, "to_state": self.current_state, "timestamp": context.get("time", 0.0)}
                )
