...
class FloaterValidator:
    ...
    def _define_constraints(self) -> Dict[str, Dict[str, Any]]:
        ...
        return {
            "volume": {"min": 0.01, "max": 10.0, "description": "Floater volume"},  # m³  # m³
            "mass": {"min": 0.1, "max": 1000.0, "description": "Floater mass"},  # kg  # kg
            "drag_coefficient": {"min": 0.0, "max": 2.0, "description": "Drag coefficient"},
            "position": {"min": 0.0, "max": 25.0, "description": "Vertical position"},  # m  # m
            "velocity": {"min": -10.0, "max": 10.0, "description": "Vertical velocity"},  # m/s  # m/s
            "air_pressure": {"min": 50000, "max": 1000000, "description": "Air pressure"},  # Pa  # Pa
        }
    ...
    def validate_parameters(self, params: Dict[str, Any]) -> ValidationResult:
        ...
        for param_name, value in params.items():
            if param_name in self.constraints:
                ...
                if value < constraint["min"]:
                    errors.append(
                        f"{constraint['description']} ({param_name}) " f"must be >= {constraint['min']}, got {value}"
                    )
                ...
                # Check maximum value
                    ...
                    errors.append(
                        f"{constraint['description']} ({param_name}) " f"must be <= {constraint['max']}, got {value}"
                    )
        ...
        return ValidationResult(is_valid=is_valid, errors=errors, warnings=warnings, corrected_values=corrected_values)
        ...
        if "volume" in params and "mass" in params:
            ...
            if volume > 0:
                ...
                if density > 1000:  # Water density
                    errors.append(f"Floater density ({density:.1f} kg/m³) " f"exceeds water density (1000 kg/m³)")
                elif density > 800:
                    warnings.append(f"High floater density ({density:.1f} kg/m³) " f"may affect buoyancy")
