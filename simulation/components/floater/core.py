...
class Floater:
    ...
    def __init__(self, config: FloaterConfig):
        ...
        self.buoyancy_calculator = BuoyancyCalculator(tank_height=config_dict.get("tank_height", 10.0))
        ...
        logger.info(
            f"Floater initialized with volume={config_dict.get('volume', 0.4)}m³, mass={config_dict.get('mass', 16.0)}kg"
        )
    ...
    def _calculate_drag(self, fluid_system=None) -> float:
        ...
        drag_force = 0.5 * 1000 * velocity_magnitude**2 * area * drag_coefficient
