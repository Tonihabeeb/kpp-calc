...
class ThermalModel:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        logger.info(f"Thermal model initialized - Water temp: {self.water_temperature:.1f} K, "
    ...
    def calculate_thermal_buoyancy_enhancement(self, base_buoyancy: float, 
                                             ascent_height: float) -> float:
        """
        ...
        if ascent_height > 0.1:  # Meaningful height change
            ...
            logger.debug(f"H2 thermal enhancement: height={ascent_height:.1f}m, "
    ...
    def calculate_isothermal_compression_work(self, volume: float, initial_pressure: float, 
        ...
        if self.h2_active:
            ...
            logger.debug(f"H2 compression work: ideal={isothermal_work:.1f} J, "
    ...
    def calculate_adiabatic_compression_work(self, volume: float, initial_pressure: float, 
                                           final_pressure: float) -> float:
        """
        ...
        logger.debug(f"Adiabatic compression work: V₁={volume:.4f} m³, V₂={final_volume:.4f} m³, "
                    f"P₁={initial_pressure:.0f} Pa, P₂={final_pressure:.0f} Pa, "
                    f"W={adiabatic_work:.1f} J")
        
        return adiabatic_work
    
    def calculate_compression_work(self, volume: float, depth: float, 
                                 use_isothermal: Optional[bool] = None) -> float:
        """
        ...
        logger.debug(f"Compression work at depth {depth:.1f}m: "
                    f"{'isothermal' if use_isothermal else 'adiabatic'} = {work:.1f} J")
        
        return work
    
    def calculate_thermal_density_effect(self, base_density: float, 
        ...
        if abs(temperature_change) > 0.1:
            logger.debug(f"Thermal density effect: ΔT={temperature_change:.1f}K, "
                        f"density: {base_density:.1f} → {adjusted_density:.1f} kg/m³")
        
        return adjusted_density
    
    def calculate_heat_exchange_rate(self, air_volume: float, temperature_difference: float, 
        ...
        logger.debug(f"Heat exchange: A={surface_area:.3f} m², ΔT={temperature_difference:.1f}K, "
                    f"Q̇={heat_rate:.1f} W")
    ...
    def set_h2_active(self, active: bool, efficiency: Optional[float] = None, 
        ...
        logger.info(f"H2 isothermal effects {'activated' if active else 'deactivated'}: "
    ...
    def set_ambient_temperature(self, temperature: float):
        ...
        logger.info(f"Ambient temperature set to {self.ambient_temperature:.1f} K "
