...
class CompressionThermodynamics:
    ...
    def adiabatic_compression_temperature(self, 
                                        T_initial: float,
                                        P_initial: float,
                                        P_final: float) -> float:
        """
        ...
        logger.debug(f"Adiabatic compression: {T_initial:.1f}K → {T_final:.1f}K "
                    f"(P: {P_initial/1000:.1f} → {P_final/1000:.1f} kPa)")
        
        return T_final
    
    def isothermal_compression_work(self, 
    ...
    def adiabatic_compression_work(self, 
                                  volume_initial: float,
                                  P_initial: float,
                                  P_final: float,
    ...
    def compression_heat_generation(self, 
                                   compression_work: float,
        ...
        logger.debug(f"Compression heat: {heat_generated/1000:.1f} kJ "
                    f"(efficiency: {efficiency:.1%})")
        
        return heat_generated
    
    def intercooling_temperature_drop(self, 
                                     T_hot: float,
                                     heat_removed: float,
                                     air_mass: float) -> float:
        """
        ...
        logger.debug(f"Intercooling: {T_hot:.1f}K → {T_final:.1f}K "
...
class ExpansionThermodynamics:
    ...
    def _adiabatic_expansion(self, 
                            volume_initial: float,
                            P_initial: float,
                            P_final: float,
                            T_initial: float) -> Tuple[float, float]:
        """
    ...
    def _isothermal_expansion(self, 
                             volume_initial: float,
                             P_initial: float,
                             P_final: float,
                             T_initial: float) -> Tuple[float, float]:
    ...
    def _mixed_expansion(self, 
                        volume_initial: float,
                        P_initial: float,
                        P_final: float,
                        T_initial: float,
                        heat_transfer_rate: float = 0.5) -> Tuple[float, float]:
        """
    ...
    def expansion_with_heat_transfer(self, 
                                   volume_initial: float,
                                   P_initial: float,
                                   P_final: float,
                                   T_initial: float,
                                   expansion_mode: str = 'mixed',
                                   expansion_time: float = 10.0) -> Dict:
        """
        ...
        heat_transfer = (self.heat_transfer_coefficient * heat_transfer_area * 
                        temperature_difference * expansion_time)
        ...
        logger.debug(f"Expansion ({expansion_mode}): "
...
class ThermalBuoyancyCalculator:
    ...
    def thermal_buoyancy_boost(self, 
                              base_buoyant_force: float,
                              thermal_expansion_results: Dict) -> Dict:
        """
        ...
        logger.debug(f"Thermal buoyancy boost: {thermal_buoyant_force:.1f}N "
                    f"({thermal_boost_percentage:.1f}% increase)")
        
        return results
    
    def thermal_efficiency_factor(self, 
                                 thermal_energy_input: float,
                                 mechanical_work_output: float) -> float:
        """
        ...
        logger.debug(f"Thermal efficiency: {efficiency:.1%} "
...
class AdvancedThermodynamics:
    ...
    def complete_thermodynamic_cycle(self, 
        ...
        expansion_results = self.expansion.expansion_with_heat_transfer(
            ...
            injection_temperature, self.expansion_mode, ascent_time)
        ...
        logger.info(f"Complete thermodynamic cycle: "
    ...
    def analyze_complete_cycle(self, initial_volume: float, injection_pressure: float, 
                             surface_pressure: float, injection_temperature: float,
                             ascent_time: float, base_buoyant_force: float) -> Dict:
        """
        ...
        return self.complete_thermodynamic_cycle(
            initial_volume, injection_pressure, surface_pressure,
