...
class PressureControlSystem:
    ...
    def control_step(self, dt: float, current_time: float) -> Dict[str, Any]:
        ...
        if (
            current_pressure > self.settings.emergency_high_pressure
            or current_pressure < self.settings.critical_low_pressure
        ):
...
def create_standard_kpp_pressure_controller(
    target_pressure_bar: float = 2.5,
