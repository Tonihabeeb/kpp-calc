...
class HeatTransferCoefficients:
    ...
    def overall_heat_transfer_coefficient(self, 
        ...
        """
        thermal_resistance = (1/inner_coefficient + 
    ...
    def nusselt_number_cylinder(self, reynolds: float, prandtl: float) -> float:
        ...
        nusselt = 0.3 + (0.62 * reynolds**0.5 * prandtl**(1/3) * re_factor) / (1 + (0.4/prandtl)**(2/3))**0.25
...
class WaterThermalReservoir:
    ...
    def thermal_stratification_effect(self, position_initial: float, 
                                    position_final: float) -> Dict:
        """
        ...
        logger.debug(f"Thermal stratification: {temp_initial:.1f}K → {temp_final:.1f}K "
...
class AirWaterHeatExchange:
    ...
    def convective_heat_transfer_coefficient(self, 
                                           ...
                                           water_velocity: float = 0.5) -> Tuple[float, float]:
        """
        ...
        if water_velocity > 0.1:
            ...
            re_water = self.heat_coeffs.reynolds_number(
                water_velocity, self.geometry['diameter'], 
                self.heat_coeffs.thermal_conductivity_water / (RHO_WATER * 4186))
            
            # Calculate Nusselt number
            nu_water = self.heat_coeffs.nusselt_number_cylinder(
                re_water, 7.0)  # Prandtl = 7 for water
            
            # Heat transfer coefficient
            h_water = (nu_water * self.heat_coeffs.thermal_conductivity_water / 
    ...
    def overall_heat_transfer_coefficient(self, 
                                        air_velocity: float = 0.0,
                                        water_velocity: float = 0.5) -> float:
        """
    ...
    def heat_transfer_rate(self, 
        ...
        logger.debug(f"Heat transfer rate: {heat_rate:.1f} W "
                    f"(ΔT: {temperature_difference:.1f}K)")
        
        return heat_rate
    
    def heat_transfer_over_time(self, 
        ...
        logger.debug(f"Heat transfer over {time_duration:.1f}s: "
...
class CompressionHeatRecovery:
    ...
    def recoverable_heat_from_compression(self, 
                                        compression_work: float,
                                        compression_efficiency: float = 0.85) -> float:
        """
        ...
        logger.debug(f"Recoverable heat: {recoverable_heat/1000:.1f} kJ "
                    f"({recoverable_fraction:.1%} of {heat_generated/1000:.1f} kJ)")
        
        return recoverable_heat
    
    def heat_recovery_to_water(self, 
                              recoverable_heat: float,
                              water_temperature: float,
                              heat_recovery_time: float) -> Dict:
        """
        ...
        if water_mass_affected > 0:
            water_temperature_rise = (recoverable_heat * self.heat_exchanger_effectiveness) / (water_mass_affected * water_specific_heat)
        else:
        ...
        logger.debug(f"Heat recovery: {actual_heat_transferred/1000:.1f} kJ → "
...
class IntegratedHeatExchange:
    ...
    def complete_heat_exchange_analysis(self, 
                                      ...
                                      ascent_time: float,
                                      compression_work: float = 0.0) -> Dict:
        """
        ...
        heat_exchange = self.air_water_exchange.heat_transfer_over_time(
            air_temperature, water_temp_average, air_mass, ascent_time,
            time_step=0.1, water_velocity=water_velocity)
        ...
        if compression_work > 0:
            ...
            heat_recovery_results = self.heat_recovery.heat_recovery_to_water(
                recoverable_heat, stratification['initial_water_temperature'], 
                ascent_time)
        ...
        results = {
            ...
            'thermal_performance': {
                ...
                'thermal_efficiency': heat_exchange['total_heat_transferred'] / max(1.0, compression_work) if compression_work > 0 else 0.0
            }
        }
        
        logger.info(f"Complete heat exchange analysis: "
