...
class PneumaticControlCoordinator:
    ...
    def __init__(
        ...
        logger.info(
            f"PneumaticControlCoordinator initialized: thermodynamics={enable_thermodynamics}, optimization={enable_optimization}"
        )
    ...
    def detect_faults(self):
        ...
        if tank_pressure < self.control_params.min_pressure:
            self.add_fault(
                ...
                f"Tank pressure too low: {tank_pressure/100000:.2f} bar",
            )
        elif tank_pressure > self.control_params.max_pressure:
            self.add_fault(
                FaultType.PRESSURE_DROP,
                f"Tank pressure too high: {tank_pressure/100000:.2f} bar",
            )
        ...
        if compressor_temp > self.control_params.thermal_shutdown_temp:
            self.add_fault(
                ...
                f"Compressor overheating: {compressor_temp-273.15:.1f}°C",
            )
    ...
    def pressure_control_algorithm(self):
        ...
        if tank_pressure < (target_pressure - tolerance):
            if not self.compressor_enabled:
                ...
                logger.info(
                    f"Compressor ON: pressure {tank_pressure/100000:.2f} bar < target {target_pressure/100000:.2f} bar"
                )
            ...
            if self.compressor_enabled:
                self.compressor_enabled = False
                logger.info(
                    f"Compressor OFF: pressure {tank_pressure/100000:.2f} bar > target {target_pressure/100000:.2f} bar"
                )
    ...
    def trigger_injection(self):
        ...
        logger.info(
            f"Injection triggered: cycle {self.cycle_count}, pressure {self.sensors.tank_pressure.value/100000:.2f} bar"
        )
...
def create_standard_kpp_pneumatic_coordinator(
    enable_thermodynamics: bool = True, enable_optimization: bool = True
