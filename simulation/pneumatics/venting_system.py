...
class VentingTrigger:
    ...
    def __init__(self, 
                 ...
                 surface_breach_depth: float = 0.1):  # Depth considered "surface breach" (m)
        """
...
class AirReleasePhysics:
    ...
    def calculate_air_release_rate(self, 
                                  internal_pressure: float,
                                  external_pressure: float) -> float:
        """
        ...
        if pressure_ratio <= critical_ratio:
            ...
            flow_rate = (self.discharge_coefficient * self.vent_valve_area * 
                        internal_pressure * math.sqrt(1.4 / (287 * 293.15)) *
            ...
            flow_rate = (self.discharge_coefficient * self.vent_valve_area * 
                        math.sqrt(2 * pressure_diff / 1.225))  # Air density ≈ 1.225 kg/m³
        
        logger.debug(f"Air release rate: {flow_rate*1000:.1f} L/s "
                    f"(P_int={internal_pressure/1000:.1f} kPa, "
                    f"P_ext={external_pressure/1000:.1f} kPa)")
        
        return max(0.0, flow_rate)
    
    def calculate_water_inflow_rate(self, 
                                   floater_air_volume: float,
        ...
        logger.debug(f"Water inflow rate: {actual_rate*1000:.1f} L/s "
    ...
    def calculate_bubble_escape_time(self, depth: float, bubble_volume: float) -> float:
        ...
        logger.debug(f"Bubble escape: {escape_time:.1f}s, "
...
class AutomaticVentingSystem:
    ...
    def check_venting_trigger(self, floater_id: str, 
                             floater_position: float,
                             floater_tilt: float = 0.0) -> bool:
        """
    ...
    def start_venting(self, floater_id: str, 
        ...
        logger.info(f"Started venting for floater {floater_id}: "
                   f"{initial_air_volume*1000:.1f}L at {initial_air_pressure/1000:.1f} kPa")
        
        return venting_state
    
    def update_venting_process(self, floater_id: str, 
        ...
        state['water_volume'] = min(state['floater_total_volume'], 
                                   state['water_volume'] + water_added_this_step)
        ...
        if state['current_air_volume'] > 0:
            ...
            state['current_air_pressure'] = max(external_pressure, 
                                               state['current_air_pressure'] + pressure_change)
        else:
            state['current_air_pressure'] = external_pressure
        ...
        logger.debug(f"Venting update {floater_id}: "
    ...
    def complete_venting(self, floater_id: str, completion_time: float) -> Dict:
        ...
        logger.info(f"Venting completed for floater {floater_id} in {venting_duration:.2f}s: "
    ...
    def get_system_status(self) -> Dict:
        ...
        completed_count = sum(1 for state in self.active_venting_floaters.values() 
                             if state['venting_complete'])
