...
class AirInjectionController:
    ...
    def __init__(
        ...
        logger.info(
            f"AirInjectionController initialized: "
    ...
    def add_injection_request(self, request: FloaterInjectionRequest) -> bool:
        ...
        logger.info(
            f"Added injection request for floater {request.floater_id}: "
    ...
    def start_injection(
        ...
        logger.info(
            f"Starting injection for floater {request.floater_id}: "
            ...
            if self.valve_state == ValveState.OPEN and self.current_flow_rate > 0:
                ...
                if volume_complete or time_complete:
                    ...
                    if volume_complete:
                        ...
                        logger.info(
                            f"Injection completed for floater {request.floater_id}: "
                            f"injected {request.injected_volume*1000:.1f}L in {injection_time:.1f}s"
                        )
                    else:
                        self.failed_injections += 1
                        logger.warning(
                            f"Injection timeout for floater {request.floater_id}: "
