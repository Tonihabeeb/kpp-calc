...
class NanobubblePhysics:
    ...
    def __init__(self, enabled: bool = False, nanobubble_fraction: float = 0.0,
                 generation_power: float = 2500.0, max_drag_reduction: float = 0.5):
        """
        ...
        logger.info(f"NanobubblePhysics initialized: enabled={enabled}, "
    ...
    def update(self, dt: float, water_temperature: float = 293.15) -> None:
        ...
        logger.debug(f"Nanobubble update: power={self.power_consumption:.1f}W, "
    ...
    def get_bubble_characteristics(self) -> Dict[str, float]:
        ...
        return {
            ...
            'power_per_bubble_W': self.power_consumption / max(1, self.nanobubble_fraction * 1e12)
        }
