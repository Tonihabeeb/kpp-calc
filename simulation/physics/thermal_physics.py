...
class ThermalPhysics:
    ...
    def __init__(self, enabled: bool = False, thermal_coefficient: float = 0.0001,
                 thermal_efficiency: float = 0.75, target_temperature: float = 298.15):
        """
        ...
        logger.info(f"ThermalPhysics initialized: enabled={enabled}, "
                   f"coefficient={thermal_coefficient:.6f}")
    
    def update(self, dt: float, ambient_temperature: float = 293.15,
               heat_input: float = 0.0) -> None:
        """
        ...
        if heat_input > 0:
            ...
            temperature_rise = heat_input * dt * self.thermal_efficiency / (
                self.water_specific_heat * 1000  # Assume 1000 kg of water
            )
            self.current_temperature = min(
                self.target_temperature,
                self.current_temperature + temperature_rise
            )
        else:
            ...
            self.current_temperature = max(
                ambient_temperature,
                self.current_temperature - cooling
            )
        ...
        logger.debug(f"Thermal update: T={self.current_temperature:.1f}K, "
    ...
    def get_temperature_effects(self) -> Dict[str, float]:
        ...
        return {
            ...
            'thermal_expansion_effect': (self.current_temperature - 293.15) * self.water_thermal_expansion,
    ...
    def calculate_required_heat_input(self, desired_temp_rise: float) -> float:
        ...
        required_heat_rate = (
            water_mass * self.water_specific_heat * desired_temp_rise / 
