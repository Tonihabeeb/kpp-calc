...
class PulseController:
    ...
    def __init__(self, enabled: bool = False, pulse_duration: float = 5.0,
                 coast_duration: float = 5.0, initial_phase: str = 'pulse'):
        """
        ...
        self.duty_cycle = self.pulse_duration / (self.pulse_duration + self.coast_duration)
        
        logger.info(f"PulseController initialized: enabled={enabled}, "
                   f"pulse={pulse_duration}s, coast={coast_duration}s, "
                   f"duty_cycle={self.duty_cycle:.2f}")
    
    def update(self, current_time: float, dt: float, system_state: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        """
        ...
        logger.debug(f"Pulse update: phase={self.current_phase}, "
    ...
    def update_timing(self, new_pulse_duration: float, new_coast_duration: float) -> None:
        """
        ...
        logger.info(f"Updated timing: pulse={self.pulse_duration}s, "
