...
from simulation.physics.losses import DrivetrainLosses, ElectricalLosses, LossComponents, ComponentState
...
class IntegratedLossModel:
    ...
    def update_system_losses(self, system_state: Dict, dt: float) -> EnhancedSystemState:
        ...
        thermal_efficiency_factors = self._calculate_thermal_efficiency_effects(component_states)
    ...
    def _calculate_enhanced_mechanical_losses(self, component_states: Dict[str, ComponentState]) -> LossComponents:
        ...
        for name, state in component_states.items():
            enhanced_efficiency = self.thermal_model.calculate_temperature_effects_on_efficiency(
    ...
    def _calculate_heat_generation(self, mechanical_losses: LossComponents, 
                                 electrical_losses: float, 
                                 component_states: Dict[str, ComponentState]) -> Dict[str, float]:
    ...
    def _calculate_performance_metrics(self, system_losses: SystemLosses, 
                                     thermal_states: Dict, 
                                     system_state: Dict) -> Dict[str, float]:
        """Calculate comprehensive performance metrics"""
        metrics = {}
        
        # Efficiency metrics
        metrics['system_efficiency'] = system_losses.system_efficiency
        metrics['mechanical_efficiency'] = 1.0 - (system_losses.mechanical_losses.total_losses / 
                                                 max(1.0, system_state.get('input_power', 1.0)))
        metrics['electrical_efficiency'] = 1.0 - (system_losses.electrical_losses / 
        ...
        if system_losses.total_system_losses > 0:
            metrics['bearing_loss_percent'] = (system_losses.mechanical_losses.bearing_friction / 
                                              system_losses.total_system_losses) * 100
            metrics['gear_loss_percent'] = (system_losses.mechanical_losses.gear_mesh_losses / 
                                           system_losses.total_system_losses) * 100
            metrics['electrical_loss_percent'] = (system_losses.electrical_losses / 
                                                 system_losses.total_system_losses) * 100
        else:
    ...
    def _check_system_warnings(self, enhanced_state: EnhancedSystemState):
        ...
        if enhanced_state.system_losses.system_efficiency < self.efficiency_warning_threshold:
            logger.warning(f"System efficiency below threshold: {enhanced_state.system_losses.system_efficiency:.3f} < {self.efficiency_warning_threshold}")
        
        # Temperature warnings
        max_temp = enhanced_state.performance_metrics.get('max_temperature', 0.0)
        if max_temp > self.temperature_warning_threshold:
            logger.warning(f"System temperature above threshold: {max_temp:.1f}°C > {self.temperature_warning_threshold}°C")
        ...
        for component, status in thermal_limits.items():
            if isinstance(status, dict) and status.get('over_limit', False):
                logger.warning(f"Component {component} over thermal limit: {status['temperature']:.1f}°C > {status['limit']:.1f}°C")
...
def create_standard_kpp_enhanced_loss_model(ambient_temp: float = 20.0) -> IntegratedLossModel:
    """
