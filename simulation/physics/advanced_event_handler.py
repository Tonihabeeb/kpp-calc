...
class AdvancedEventHandler:
    ...
    def __init__(self, tank_depth=10.0, optimization_params=None):
        ...
        logger.info(f"AdvancedEventHandler initialized: depth={tank_depth}m, "
    ...
    def handle_injection(self, floater, floater_id=None, current_time=0.0):
        ...
        logger.info(f"Advanced injection: floater_id={floater_id}, "
    ...
    def _can_inject(self, floater, floater_id):
        ...
        return (self._is_at_bottom(floater) and 
                self._is_heavy(floater) and 
                floater_id not in self.processed_injection)
    
    def _can_vent(self, floater, floater_id):
        """Check if venting is possible."""
        return (self._is_at_top(floater) and 
                self._is_light(floater) and 
                floater_id not in self.processed_venting)
    ...
    def _update_injection_tracking(self, floater_id, time, energy):
        ...
        self.injection_history[floater_id].append({
            'time': time,
            'energy': energy
        })
    ...
    def _update_energy_optimization(self):
        ...
        if len(self.energy_per_injection) >= 5:  # Need some data
            ...
            energy_variance = sum((e - self.average_injection_energy)**2 for e in recent_energies) / len(recent_energies)
    ...
    def _calculate_system_efficiency(self):
        ...
        if len(self.energy_per_injection) > 1:
            energy_std = math.sqrt(sum((e - self.average_injection_energy)**2 
                                     for e in self.energy_per_injection) / len(self.energy_per_injection))
    ...
    def _check_cycle_completion(self, floater_id):
        ...
        return (floater_id in self.injection_history and 
                floater_id in self.venting_history and
                len(self.injection_history[floater_id]) == len(self.venting_history[floater_id]))
