...
class LoadCurtailmentController:
    ...
    def request_curtailment(self, reduction_mw: float, duration_s: float, 
    ...
    def _validate_curtailment_request(self, reduction_mw: float, duration_s: float, 
                                    event_type: CurtailmentType, current_time: float) -> bool:
        ...
        if (duration_s < self.config.min_curtailment_duration or 
            duration_s > self.config.max_curtailment_duration):
            return False
    ...
    def _check_for_curtailment_needs(self, current_load: float, grid_conditions: Dict[str, Any], 
        ...
        freq_emergency = (emergency_conditions.get('grid_frequency_low', False) or 
                         emergency_conditions.get('grid_frequency_high', False) or
                         frequency < 59.5 or frequency > 60.5)
        
        voltage_emergency = (emergency_conditions.get('voltage_low', False) or
                           emergency_conditions.get('voltage_high', False))
    ...
    def _update_load_tracking(self, current_load: float, current_time: float):
        ...
        non_curtailed_loads = [entry['load'] for entry in self.load_history 
                             if not entry['curtailed']]
    ...
    def _get_status(self) -> str:
        ...
        if self.curtailment_active and self.active_event:
            ...
            return (f"Curtailment active - {self.active_event.event_type.value} "
