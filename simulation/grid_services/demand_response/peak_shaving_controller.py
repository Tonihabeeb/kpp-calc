...
class PeakShavingController:
    ...
    def _update_load_forecast(self, external_forecast: Optional[List[float]] = None):
        ...
        for h in range(24):
            ...
            similar_periods = [
                entry['average'] for entry in self.hourly_averages
    ...
    def _evaluate_peak_shaving_need(self, demand: float, generation: float, 
        ...
        # Check for predicted peaks (predictive shaving)
            ...
            if upcoming_peak and not self.shaving_active:
                ...
                if (upcoming_peak.predicted_time - current_time) <= 1800:  # 30 minutes
                    if self.shaving_events_today < self.config.max_shaving_events_per_day:
                        self._start_peak_shaving(upcoming_peak.predicted_peak, current_time, 
                                               is_predicted=True, peak_event=upcoming_peak)
    ...
    def _start_peak_shaving(self, peak_demand: float, current_time: float, 
        ...
        peak_reduction = max(0.0, peak_demand - (self.historical_peak * self.config.shaving_target_percent))
    ...
    def _update_active_shaving(self, demand: float, current_time: float, dt: float):
        ...
        self.active_peak_event.shaving_amount = max(0.0, 
            self.active_peak_event.predicted_peak - demand)
        
        # Check if peak period has ended
        peak_ended = (
            (current_time - self.active_peak_event.start_time) > self.config.minimum_peak_duration and
            demand < (self.peak_threshold * 0.95)  # 5% below threshold
        )
    ...
    def _end_peak_shaving(self, current_time: float):
        ...
        if self.active_peak_event.predicted_peak > 0:
            shaving_effectiveness = (self.active_peak_event.shaving_amount / 
    ...
    def _get_next_predicted_peak(self, current_time: float) -> Optional[PeakEvent]:
        ...
        upcoming_peaks = [
            p for p in self.predicted_peaks 
    ...
    def get_performance_metrics(self) -> Dict[str, float]:
        ...
        if completed_predictions:
            accuracy_errors = [
                ...
                for p in completed_predictions if p.predicted_peak > 0
            ]
        ...
        if shaved_peaks:
            avg_shaving_effectiveness = statistics.mean([
