...
class SecondaryFrequencyController:
    ...
    def process_agc_signal(self, agc_signal: AGCSignal, dt: float) -> Dict[str, Any]:
        ...
        accuracy_threshold = self.config.accuracy_requirement * abs(self.target_response) if abs(self.target_response) > 0.001 else self.config.accuracy_requirement * self.config.regulation_capacity
        ...
        is_settled = (response_error <= accuracy_threshold) or (response_error < 0.002)  # Within 0.2% is considered settled
    ...
    def get_performance_metrics(self) -> Dict[str, float]:
        ...
        avg_regulation_time = (self.total_regulation_time / self.regulation_count 
                             if self.regulation_count > 0 else 0.0)
        ...
        if self.regulation_count > 0:
            accuracy_rate = max(0.0, (1.0 - (self.accuracy_violations / total_regulation_updates)) * 100.0)
        else:
        ...
        for i in range(1, len(self.response_history)):
            if (abs(self.response_history[i]['target']) > 0.001 and 
                abs(self.response_history[i-1]['target']) <= 0.001):
