...
class SyntheticInertiaController:
    ...
    def update(self, grid_frequency: float, dt: float, rated_power: float) -> Dict[str, Any]:
        ...
        if self.inertia_active:
            ...
            if (response_duration > self.config.response_duration or 
                abs(rocof) < self.config.rocof_threshold * 0.5):  # Hysteresis
    ...
    def _calculate_rocof(self) -> float:
        ...
        window_measurements = [
            m for m in self.frequency_buffer 
    ...
    def _create_response_dict(self, power_command: float, status: str) -> Dict[str, Any]:
        ...
        return {
            ...
            'response_duration': (time.time() - self.response_start_time 
                                if self.response_start_time else 0.0),
