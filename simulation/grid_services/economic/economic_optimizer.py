...
class EconomicOptimizer:
    ...
    def _calculate_service_values(
        self, conditions: Dict[str, Any], forecasts: Dict[str, Any]
        ...
        frequency_value = (
            freq_params.revenue_rate
            * frequency_urgency
            * freq_params.reliability_factor
            * freq_params.performance_bonus
        )
        ...
        if grid_emergency:
            ...
            stabilization_value = (
                stab_params.revenue_rate
                * 2.0  # High value during emergencies
                * stab_params.reliability_factor
                * stab_params.performance_bonus
            )
        else:
    ...
    def _perform_optimization(
        self,
        service_values: Dict[ServiceType, float],
        available_services: Dict[str, Any],
    ...
    def _optimize_balanced(
        ...
        emergency_conditions = (
            abs(conditions.get("frequency", 50.0) - 50.0) > 0.1
            or abs(conditions.get("voltage", 1.0) - 1.0) > 0.05
            or not conditions.get("grid_connected", True)
        )
    ...
    def _calculate_expected_revenue(
        self,
        allocation: Dict[ServiceType, float],
        service_values: Dict[ServiceType, float],
    ) -> float:
    ...
    def _generate_service_commands(
        self, allocation: Dict[ServiceType, float], conditions: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    def _update_economic_state(
        ...
        if services.get("frequency_active", False):
            ...
            freq_revenue = (
                abs(freq_power)
                * self.service_parameters[ServiceType.FREQUENCY_REGULATION].revenue_rate
                * dt
                / 3600000.0
            )
