...
class GridServicesCoordinator:
    ...
    def _update_voltage_services(
        ...
        dvs_response = self.dynamic_voltage_support.update(voltage_pu, dt, rated_power)
        ...
        # Voltage regulator (if no dynamic support active)
            ...
            vr_response = self.voltage_regulator.update(voltage_pu, dt, rated_power)
        ...
        if lc_response["curtailment_active"]:
            ...
            if curtailment_power > 0.001:  # Only if significant curtailment
                ...
                priority = (
                    ...
                    if lc_response.get("curtailment_type") == "emergency"
                    else ServicePriority.DEMAND_RESPONSE
                )
                ...
                command = ServiceCommand(
                    ...
                    priority=priority,
                    duration=lc_response.get("duration", 0.0),
                    confidence=0.90,
                )
                self.service_commands.append(command)
    ...
    def get_service_status(self) -> Dict[str, bool]:
        ...
        return {
            ...
            "peak_shaving": self.peak_shaving_controller.is_shaving(),
            "load_forecaster": self.load_forecaster.is_forecasting(),
            ...
            "market_interface": self.config.enable_economic_optimization
            and len(getattr(self.market_interface, "active_bids", {})) > 0,
