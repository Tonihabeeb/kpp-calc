...
class GridStabilizationController:
    ...
    def _determine_stabilization_mode(self, conditions: GridConditions, battery_status: Dict[str, Any]):
        ...
        if (
            abs(conditions.frequency - 50.0) > 1.0
            or abs(conditions.voltage - 1.0) > 0.15
            or abs(conditions.rocof) > 2.0
        ):
    ...
    def _execute_stabilization_control(
        self, conditions: GridConditions, battery_status: Dict[str, Any], dt: float
    ) -> Dict[str, Any]:
    ...
    def _frequency_support_control(
        self, conditions: GridConditions, battery_status: Dict[str, Any], dt: float
    ) -> Dict[str, Any]:
    ...
    def set_control_parameters(
        self,
        frequency_droop: Optional[float] = None,
        voltage_droop: Optional[float] = None,
...
def create_grid_stabilization_controller(
    max_power_kw: float = 250.0, response_time_ms: float = 500.0
