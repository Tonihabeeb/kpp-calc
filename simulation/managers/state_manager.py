...
from ..schemas import (
    SimulationState, EnergyLossData, PerformanceMetrics, SystemState,
...
class StateManager(BaseManager):
    ...
    def build_system_state(self, total_vertical_force: float,
                          base_buoy_force: float,
                          enhanced_buoy_force: float,
                          thermal_enhanced_force: float,
                          pulse_force: float,
                          preliminary_state: Dict[str, Any]) -> SystemState:
        """
    ...
    def build_simulation_output(self, system_state: SystemState,
                              ...
                              grid_services_response: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        """
    ...
    def _update_energy_losses(self) -> None:
        ...
        self.energy_losses = EnergyLossData(
            drag_loss=drag_loss,
            dissolution_loss=dissolution_loss,
    ...
    def get_performance_metrics(self) -> Dict[str, float]:
        ...
        mechanical_power = abs(
            getattr(self.engine, "final_output_torque", 0.0) *
            getattr(self.engine, "final_output_speed", 0.0)
        )
        ...
        force_utilization = (
            getattr(self.engine, "total_vertical_force", 0.0) / total_buoyant_force 
            if total_buoyant_force > 0 else 0.0
        )
        
        # Calculate energy loss rates
        total_energy_loss = (self.energy_losses.drag_loss + 
        ...
        return {
            ...
            "dissolution_loss_ratio": self.energy_losses.dissolution_loss / total_energy_loss if total_energy_loss > 0 else 0.0,
    ...
    def collect_and_log_state(self, dt: float, physics_results: PhysicsResults, 
                             system_results: SystemResults) -> SimulationState:
        """
        ...
        drivetrain_data = system_results.drivetrain
        electrical_data = system_results.electrical
        control_data = system_results.control
