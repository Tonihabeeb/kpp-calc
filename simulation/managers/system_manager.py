...
class SystemManager(BaseManager):
    ...
    def update_electrical_system(
        self, preliminary_torque: float, preliminary_speed_rad_s: float, dt: float, control_commands: Dict[str, Any]
    ) -> ElectricalSystemOutput:
        """
        ...
        control_commands.get("timing_commands", {})
        ...
        electrical_config_updates = {
            ...
            "power_setpoint": load_commands.get("power_setpoint", self.get_config_param("target_power", 530000.0)),
    ...
    def update_drivetrain_with_load(self, electrical_load_torque: float, dt: float) -> Dict[str, Any]:
        """
        Update the integrated_drivetrain system with actual electrical load feedback.
        ...
        Returns:
            Final integrated_drivetrain output data
        """
        # Update integrated_drivetrain with load feedback
        drivetrain_output = self.engine.integrated_drivetrain.update(
            self.engine.chain_tension, electrical_load_torque, dt
        )

        logger.debug(f"IntegratedDrivetrain updated with load: {electrical_load_torque:.1f}Nm")
    ...
    def update_transient_events(
        self, system_state: Dict[str, Any], electrical_output: Dict[str, Any], preliminary_state: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
    ...
    def update_enhanced_loss_model(
        self,
        final_output_torque: float,
        ...
        Args:
            final_output_torque: Final integrated_drivetrain output torque
            final_output_speed: Final integrated_drivetrain output speed
        ...
        enhanced_state = self.engine.enhanced_loss_model.update_system_losses(enhanced_system_state, dt)
    ...
    def update_grid_services(self, electrical_output: Dict[str, Any], grid_power_output: float) -> Dict[str, Any]:
        """
    ...
    def update_pneumatic_performance_tracking(self, dt: float) -> None:
        ...
        Args:
            dt: Time step in seconds
        """
        if hasattr(self.engine, "pneumatic_coordinator") and hasattr(self.engine, "pneumatic_performance_analyzer"):
            ...
            chain_speed = getattr(self.engine.integrated_drivetrain, "omega_chain", 0.0) * self.engine.params.get(
                "sprocket_radius", 1.0
            )
    ...
    def update_systems(self, dt: float, physics_results: PhysicsResults) -> SystemResults:
        """
        Update all system components including integrated_drivetrain, electrical, control, and grid services.
        ...
        # 1. Calculate preliminary integrated_drivetrain torque for electrical system
        ...
        preliminary_speed = 0.0  # Will be calculated from integrated_drivetrain
        electrical_output = self.update_electrical_system(preliminary_torque, preliminary_speed, dt, basic_control)
        electrical_load_torque = electrical_output.electrical_load_torque

        # 3. Update integrated_drivetrain with actual electrical load
        ...
        system_state = {
            ...
            "integrated_drivetrain": drivetrain_output,
        ...
        transient_response = self.update_transient_events(
            system_state, electrical_output.model_dump(), preliminary_state
        )
        ...
        system_results = SystemResults(
            integrated_drivetrain=self._convert_drivetrain_to_schema(drivetrain_output),
    ...
    def _convert_drivetrain_to_schema(self, drivetrain_output: Dict[str, Any]):
        """Convert integrated_drivetrain output to DrivetrainData schema."""
