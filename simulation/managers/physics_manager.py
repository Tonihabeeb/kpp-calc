...
class PhysicsManager(BaseManager):
    ...
    def calculate_floater_forces(self, dt: float) -> Dict[str, float]:
        ...
        if hasattr(self.engine, "pulse_controller"):
            ...
            pulse_state = {"active": False, "power": 0.0, "efficiency": 0.92}
        ...
        for i, floater in enumerate(self.engine.floaters):
            ...
            current_chain_speed = (
                self.engine.chain_system.get_chain_speed() if hasattr(self.engine, "chain_system") else 2.0
            )
    ...
    def _calculate_single_floater_forces(
        ...
        h1_enhanced_buoyancy, nanobubble_state = self._apply_h1_nanobubble_physics(base_buoyancy, floater)
        h1_enhancement = h1_enhanced_buoyancy - base_buoyancy

        # === H2 THERMAL PHYSICS ===
        h2_enhanced_buoyancy = self._apply_h2_thermal_physics(base_buoyancy, is_ascending, y, dt)
        h2_enhancement = h2_enhanced_buoyancy - base_buoyancy

        # === H3 PULSE CONTROL ===
        h3_pulse_force = self._apply_h3_pulse_physics(floater, pulse_state)
        ...
        thermal_buoyancy = self.engine.thermal_model.calculate_thermal_buoyancy_enhancement(
            fluid_buoyancy, ascent_height
        )

        # Enhanced drag calculation
        drag_force = self._calculate_drag_force(nanobubble_state, chain_speed, floater, y)

        # Calculate net vertical force for this floater
        net_force = self._calculate_net_force(floater, combined_buoyancy, drag_force, h3_pulse_force)
    ...
    def _calculate_drag_force(self, nanobubble_state, chain_speed: float, floater, y: float) -> float:
        """Calculate drag force with nanobubble effects."""
        if nanobubble_state and nanobubble_state.active:
            # H1: Reduced drag
            base_drag = self.engine.fluid_system.calculate_drag_force(chain_speed, floater.area)
            drag_force = base_drag * (1 - nanobubble_state.drag_reduction)
        else:
            drag_force = self.engine.fluid_system.calculate_drag_force(chain_speed, floater.area)
    ...
    def _calculate_net_force(self, floater, combined_buoyancy: float, drag_force: float, pulse_force: float) -> float:
        ...
        if getattr(floater, "is_filled", False):
            ...
            water_weight = (
                floater.volume * self.engine.fluid_system.state.effective_density * self.engine.fluid_system.gravity
            )
            net_force = -(floater_weight + water_weight) + drag_force + pulse_force

        return net_force

    def _log_floater_physics(
        self,
        floater_idx: int,
        x: float,
        y: float,
        ...
        for i, floater in enumerate(self.engine.floaters):
            ...
            floater_data.append(
                FloaterPhysicsData(
                    ...
                    pulse_force=(
                        floater.compute_pulse_jet_force() if hasattr(floater, "compute_pulse_jet_force") else 0.0
                    ),
