...
class KPPSynchronizedSystemLauncher:
    ...
    def wait_for_server(self, config, timeout=30):
        ...
        while time.time() - start_time < timeout:
            try:
                ...
                if response.status_code == 200:
                    logger.info(f"[OK] {config['name']} is healthy!")
                    return True
        ...
        logger.error(f"[FAIL] {config['name']} failed to start within {timeout}s")
        return False
    
    def start_server(self, config):
        """Start a single server"""
        logger.info(f"[START] Starting {config['name']}...")
        
        # Check if port is available
        if not self.check_port_available(config['port']):
            logger.warning(f"[WARN] Port {config['port']} is already in use")
            
            # Check if the service is actually healthy
            try:
                response = requests.get(config['health_url'], timeout=2)
                if response.status_code == 200:
                    logger.info(f"[OK] {config['name']} is already running and healthy!")
                    return "existing"  # Return special value to indicate existing service
                else:
                    logger.warning(f"[WARN] {config['name']} port in use but not responding")
            except:
                logger.warning(f"[WARN] {config['name']} port in use but not responding")
        ...
        try:
            ...
            if process.poll() is None:
                logger.info(f"[OK] {config['name']} process started (PID: {process.pid})")
                return process
            else:
                stdout, stderr = process.communicate()
                logger.error(f"[FAIL] {config['name']} failed to start")
                logger.error(f"stdout: {stdout.decode()}")
                logger.error(f"stderr: {stderr.decode()}")
                return None
                
        except Exception as e:
            logger.error(f"[FAIL] Failed to start {config['name']}: {e}")
            return None
    
    def start_all_servers(self):
        """Start all servers in the correct order"""
        logger.info("[SYNC] Starting KPP Synchronized System...")
        logger.info("=" * 60)
        ...
        for config in self.server_configs:
            ...
            if result == "existing":
                ...
                success_count += 1
                logger.info(f"[OK] {config['name']} already running and healthy!")
            elif result:
                # New service started successfully
                if self.wait_for_server(config):
                    success_count += 1
                else:
                    logger.error(f"[FAIL] {config['name']} failed health check")
            else:
                logger.error(f"[FAIL] {config['name']} failed to start")
        ...
        if success_count == len(self.server_configs):
            logger.info("[SUCCESS] All servers started successfully!")
            self.print_system_status()
            return True
        else:
            logger.error(f"[FAIL] Only {success_count}/{len(self.server_configs)} servers started successfully")
            return False
    
    def print_system_status(self):
        """Print the current system status"""
        logger.info("\n[STATUS] KPP Synchronized System Status:")
        logger.info("-" * 50)
        
        for config in self.server_configs:
            try:
                response = requests.get(config['health_url'], timeout=2)
                status = "[GREEN] HEALTHY" if response.status_code == 200 else "[YELLOW] DEGRADED"
            except:
                status = "[RED] OFFLINE"
    ...
    def stop_all_servers(self):
        ...
        logger.info("[STOP] Stopping all servers...")
        ...
        for server_info in self.processes:
            try:
                ...
                if process.poll() is None:  # Process is still running
                    ...
                    logger.info(f"[OK] {server_info['name']} stopped")
...
def main():
    ...
    try:
        ...
        if launcher.start_all_servers():
            logger.info("\n[TARGET] System ready for real-time synchronized operation!")
            ...
            while True:
                ...
                try:
                    ...
                    if healthy_count < len(launcher.server_configs):
                        logger.warning(f"[WARN] Only {healthy_count}/{len(launcher.server_configs)} servers healthy")
                    
                except KeyboardInterrupt:
                    break
        
        else:
            logger.error("[FAIL] Failed to start system")
