...
def test_voltage_regulator():
    ...
    for i, voltage_pu in enumerate(voltage_profile):
        ...
        print(f"{i*dt:5.1f} {voltage_pu:7.3f} {response['reactive_power_mvar']:11.2f} {response['status'][:24]}")
...
def test_power_factor_controller():
    ...
    print(f"{'Case':<6} {'P (pu)':<7} {'Q (pu)':<7} {'PF':<6} {'Q Cmd':<8} {'Status':<20}")
    print("-" * 60)
    ...
    for i, (active_pu, reactive_pu) in enumerate(test_cases):
        ...
        print(f"{i+1:5d} {active_pu:6.2f} {reactive_pu:6.2f} {power_factor:5.2f} {response['reactive_power_mvar']:7.1f} {response['status'][:19]}")
...
def test_dynamic_voltage_support():
    ...
    for i, voltage_pu in enumerate(voltage_profile):
        ...
        if i % 20 == 0 or response['event_detected'] != event_detected:
            ...
            print(f"{i*dt:5.2f} {voltage_pu:7.3f} {response['reactive_power_mvar']:11.2f} {str(response['event_detected']):9s} {response['status'][:14]}")
...
def test_service_coordination():
    ...
    print(f"{'Scenario':<20} {'V Reg':<8} {'PF Ctrl':<8} {'Dyn Sup':<8} {'Priority':<12}")
    print("-" * 70)
    
    for scenario in test_scenarios:
        # Test each service response
        vr_response = regulator.update(scenario['voltage_pu'], dt, rated_power)
        pf_response = pf_controller.update(scenario['active_pu'], scenario['reactive_pu'], dt, rated_power, vr_response['regulation_active'])
        ...
        print(f"{scenario['name']:<20} {str(vr_response['regulation_active']):<8} {str(pf_response['control_active']):<8} {str(dv_response['support_active']):<8} {priority:<12}")
