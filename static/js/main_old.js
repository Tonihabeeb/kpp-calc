document.addEventListener('DOMContentLoaded', function() {
    ...
    const powerCtx = document.getElementById('powerChart').getContext('2d');    torqueChart = new Chart(torqueCtx, {
        type: 'line',
    ...
    powerChart = new Chart(powerCtx, {
        ...
        fetch('/data/history').then(r => r.json()).then(data => {
            ...
            const formattedTimeLabels = data.time.map(t => Number(t).toFixed(2));
            torqueChart.data.labels = formattedTimeLabels;
            torqueChart.data.datasets[0].data = data.torque;
            torqueChart.update();
            powerChart.data.labels = formattedTimeLabels;
