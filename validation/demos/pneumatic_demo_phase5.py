...
def demo_compression_thermodynamics():
    ...
    T_adiabatic = compression.adiabatic_compression_temperature(P_initial, P_final, T_initial)
    work_isothermal = compression.isothermal_compression_work(volume, P_initial, P_final)
    work_adiabatic = compression.adiabatic_compression_work(volume, P_initial, P_final, T_initial)
...
def demo_heat_exchange():
    ...
    print(f"  Water thermal conductivity: {reservoir.water_thermal_conductivity:.1f} W/(m·K)")
    ...
    analysis = heat_exchange.complete_heat_exchange_analysis(
        floater_position, air_volume, air_pressure, air_temp, 
        ascent_velocity, ascent_time)
...
def demo_complete_thermodynamic_cycle():
    ...
    print(f"    Thermal contribution: {performance['thermal_contribution_percent']:.1f}%")
...
def create_thermodynamic_plots():
    ...
    for ratio in pressure_ratios:
        ...
        adi_work = compression.adiabatic_compression_work(volume, P_initial, P_final, T_initial)
    ...
    for ratio in pressure_ratios:
        P_final = P_initial * ratio
        T_final = compression.adiabatic_compression_temperature(P_initial, P_final, T_initial)
    ...
    for T_air in air_temps:
        boost = calc.calculate_thermal_buoyancy_boost(air_volume, T_air, water_temp, depth)
    ...
    ax3.plot(air_temps - 273.15, buoyancy_boosts, 'r-', label='Thermal Boost', linewidth=2)
    ax3.plot(air_temps - 273.15, base_buoyancy, 'b--', label='Base Buoyancy', linewidth=2)
