...
def demonstrate_phase3_features():
    ...
    if success:
        print(f"   ✓ Injection started: {target_air*1000:.1f}L at {injection_pressure/1000:.1f} kPa")
    ...
    print(f"   {'Time':<6} {'Pos':<8} {'Depth':<8} {'Press':<8} {'AirVol':<8} {'Buoy':<8} {'ExpRatio':<8}")
    print(f"   {'(s)':<6} {'(m)':<8} {'(m)':<8} {'(bar)':<8} {'(L)':<8} {'(N)':<8} {'(-)':<8}")
    print(f"   {'-'*64}")
    ...
    for step in range(50):  # 10 seconds of simulation
        ...
        depth = floater.pressure_physics.get_depth_from_position(position, floater.tank_height)
        ...
        expanded_volume = floater.expansion_state.get('expanded_volume', floater.total_air_injected)
        
        # Print status every few steps
        if step % 5 == 0:
            print(f"   {simulation_time:<6.1f} {position:<8.2f} {depth:<8.2f} {current_pressure/100000:<8.2f} "
        ...
        if position >= 9.5:
            print(f"   {simulation_time:<6.1f} {floater.position:<8.2f} {depth:<8.2f} {current_pressure/100000:<8.2f} "
                  f"{expanded_volume*1000:<8.1f} {buoyancy:<8.1f} {expansion_ratio:<8.3f}")
    ...
    final_depth = floater.pressure_physics.get_depth_from_position(floater.position, floater.tank_height)
    ...
    # Dissolution effects
    print(f"   Dissolved air fraction: {floater.dissolved_air_fraction_enhanced*100:.2f}%")
