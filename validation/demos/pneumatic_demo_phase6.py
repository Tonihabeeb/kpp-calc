...
def run_phase6_demo():
    ...
    print(
        f"   - Performance optimization: {'Enabled' if coordinator.optimization_enabled else 'Disabled'}"
    )
    ...
    print(
        f"   - Energy consumption: {performance.get('energy_consumption', 0.0):.1f} kJ"
    )
    ...
    print(
        f"   - All subsystems stopped: {all(not s.is_active for s in coordinator.subsystems.values())}"
    )
...
def run_control_timing_demo():
    ...
    print(
        f"Timing accuracy: {100 * actual_frequency / coordinator.control_frequency:.1f}%"
    )
