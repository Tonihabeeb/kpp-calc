...
def test_phase5_integration():
    ...
    if coordinator.thermodynamics:
        ...
        expansion_data = coordinator.thermodynamics.expansion.expansion_with_heat_transfer(
            0.01, 250000.0, 150000.0, temp, 'adiabatic', 5.0
        )
        print(f"✓ Final volume: {expansion_data['final_volume']*1000:.1f} L")
        print(f"✓ Final temperature: {expansion_data['final_temperature'] - 273.15:.1f}°C")
    
    if coordinator.heat_exchange:
        print("✓ Heat exchange module loaded")
        
        # Test heat transfer calculation - use the correct method
        air_mass = coordinator.thermodynamics.props.air_mass_from_volume(0.01, pressure, temp)
        heat_exchange_data = coordinator.heat_exchange.air_water_exchange.heat_transfer_over_time(
            320.15, 288.15, air_mass, 1.0  # 47°C air, 15°C water, air mass, 1 second
        )
        print(f"✓ Air-water heat transfer: {heat_exchange_data['total_heat_transferred']/1000:.2f} kJ")
