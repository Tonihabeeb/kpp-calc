...
def demonstrate_venting_triggers():
    ...
    for pos in positions:
        should_vent = venting_system.trigger.should_trigger_venting(pos, water_depth=10.0)
        depth_from_surface = 10.0 - pos
        status = "TRIGGER" if should_vent else "no trigger"
        print(f"   Position {pos:4.1f}m (depth from surface: {depth_from_surface:.1f}m): {status}")
...
def demonstrate_air_release_physics():
    ...
    for p_int, p_ext in pressure_scenarios:
        ...
        print(f"   {p_int/1000:8.1f} kPa     | {p_ext/1000:8.1f} kPa      | {flow_rate*1000:6.1f} L/s | {flow_type}")
    ...
    for depth in depths:
        ...
        inflow_rate = air_physics.calculate_water_inflow_rate(air_volume, floater_total_volume, depth)
        
        print(f"   {depth:4.1f}m | {air_volume*1000:6.1f} L    | {available_space*1000:10.1f} L     | {inflow_rate*1000:6.1f} L/s")
...
def demonstrate_complete_venting_cycle():
    ...
    while current_time < max_time:
        ...
        if int(current_time * 10) % 5 == 0:  # Every 0.5 seconds
            ...
            print(f"   {current_time:4.1f}s | {floater.total_air_injected*1000:6.1f} L   | {floater.water_mass:6.1f} kg  | {floater.current_air_pressure/1000:8.1f} kPa | {complete_status}")
...
def demonstrate_multiple_floater_coordination():
    ...
    for i, floater in enumerate(floaters):
        print(f"   Floater {i+1}: {floater.total_air_injected*1000:.1f}L air, "
...
def plot_venting_dynamics(time_steps, air_volumes, water_masses, air_pressures):
    ...
    ax3.axhline(y=101.325, color='k', linestyle='--', alpha=0.7, label='Atmospheric Pressure')
    ...
    if len(air_volumes) > 1:
        ...
        ax4.annotate('', xy=(air_volumes[-1], water_masses[-1]), 
                    xytext=(air_volumes[0], water_masses[0]),
                    arrowprops=dict(arrowstyle='->', color='purple', lw=2))
        ax4.text(air_volumes[0], water_masses[0], 'Start', fontsize=10, 
                bbox=dict(boxstyle="round,pad=0.3", facecolor="yellow", alpha=0.7))
        ax4.text(air_volumes[-1], water_masses[-1], 'End', fontsize=10,
                bbox=dict(boxstyle="round,pad=0.3", facecolor="lightgreen", alpha=0.7))
    
    plt.tight_layout()
    plt.savefig('static/plots/phase4_venting_dynamics.png', dpi=300, bbox_inches='tight')
...
def main():
    ...
    try:
        ...
        time_data, air_data, water_data, pressure_data = demonstrate_complete_venting_cycle()
