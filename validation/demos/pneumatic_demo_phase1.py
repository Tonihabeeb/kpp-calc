...
def demonstrate_compression_physics():
    ...
    for depth in depths:
        ...
        print(f"  {depth:2d}m depth: {required_pressure/100000:.2f} bar "
    ...
    for depth in [10, 20]:
        ...
        isothermal = compressor.calculate_isothermal_compression_work(volume, target_pressure)
        adiabatic = compressor.calculate_adiabatic_compression_work(volume, target_pressure)
...
def demonstrate_pressure_control():
    ...
    print(f"High setpoint:   {control_system.settings.high_pressure_setpoint/100000:.1f} bar")
    print(f"Low setpoint:    {control_system.settings.low_pressure_setpoint/100000:.1f} bar")
    ...
    while current_time < max_time:
        ...
        power_consumption.append(results['compressor_results'].get('power_consumed', 0) / 1000.0)  # kW
    ...
    print(f"  Total runtime:     {final_status['total_runtime_hours']*3600:.0f} seconds")
...
def demonstrate_air_consumption_regulation():
    ...
    for t in range(60, 150):  # 90 seconds of operation
        ...
        if t in consumption_events:
            ...
            print(f"  t={t-60:2.0f}s: Consumed {consumption_volume*1000:.0f}L, "
...
def demonstrate_safety_systems():
    ...
    for pressure, description in test_pressures:
        ...
        print(f"  {pressure/100000:.1f} bar ({description}): {safety_level.value.upper()}")
    ...
    print(f"  After reset:           Emergency active = {control_system.emergency_stop_active}")

def create_visualization(times, pressures, compressor_states, power_consumption=None, consumption_markers=None):
    """Create visualization of pneumatic system operation."""
    try:
        fig, axes = plt.subplots(2 if power_consumption else 1, 1, figsize=(12, 8 if power_consumption else 6))
        ...
        if consumption_markers:
            for marker_time in consumption_markers:
                axes[0].axvline(x=marker_time, color='red', linestyle='--', alpha=0.7, label='Air Consumption')
        
        # Color background based on compressor state
        state_colors = {'off': 'lightgray', 'starting': 'yellow', 'running': 'lightgreen', 'stopping': 'orange'}
        ...
        if power_consumption:
            axes[1].plot(times, power_consumption, 'r-', linewidth=2, label='Compressor Power')
