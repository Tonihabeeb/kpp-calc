...
def run_phase6_demo():
    ...
    print(f"   - Optimization parameters: {coordinator.control_params.power_optimization_enabled}")
...
def run_thermodynamic_integration_demo():
    ...
    optimal_pressure = coordinator.calculate_optimal_pressure(temp_compressor, temp_water)
    ...
    if coordinator.thermodynamics:
        ...
        if 'expansion' in cycle_data and 'work_output' in cycle_data['expansion']:
            print(f"  - Work output: {cycle_data['expansion']['work_output']/1000:.2f} kJ")
        if 'compression' in cycle_data and 'adiabatic_work' in cycle_data['compression']:
            print(f"  - Compression work: {cycle_data['compression']['adiabatic_work']/1000:.2f} kJ")
        if 'thermal_buoyancy' in cycle_data and 'thermal_boost_percentage' in cycle_data['thermal_buoyancy']:
            print(f"  - Thermal boost: {cycle_data['thermal_buoyancy']['thermal_boost_percentage']:.1f}%")
        if 'energy_balance' in cycle_data and 'thermal_efficiency' in cycle_data['energy_balance']:
            print(f"  - Thermal efficiency: {cycle_data['energy_balance']['thermal_efficiency']:.4f}")
