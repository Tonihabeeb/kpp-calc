...
def test_phase6_integration_summary():
    ...
    print(
        f"✓ Safety limits: {params.min_pressure/100000:.1f} - {params.max_pressure/100000:.1f} bar"
    )
    print(
        f"✓ Injection timing: {params.injection_duration}s duration, {params.injection_delay}s delay"
    )
    print(
        f"✓ Thermal limits: {params.max_compressor_temp - 273.15:.0f}°C max compressor temp"
    )
