...
def analyze_integration_gaps():
    ...
    advanced_systems = {
        ...
        "Enhanced Loss Model": [
            "simulation/physics/integrated_loss_model.py"
        ]
    }
    ...
    if engine_file.exists():
        ...
        imports_check = {
            "integrated_drivetrain": "from simulation.components.integrated_drivetrain import" in engine_content,
            "integrated_electrical": "from simulation.components.integrated_electrical_system import" in engine_content,
            "grid_services": "from simulation.grid_services import" in engine_content,
            "integrated_control": "from simulation.control.integrated_control_system import" in engine_content,
            "loss_model": "from simulation.physics.integrated_loss_model import" in engine_content
        }
        
        # Check what's actually used in step() method
        usage_check = {
            "integrated_drivetrain": "self.integrated_drivetrain.update(" in engine_content,
            "integrated_electrical": "self.integrated_electrical_system.update(" in engine_content,
            "grid_services": "self.grid_services_coordinator.update(" in engine_content,
            "integrated_control": "self.integrated_control_system.update(" in engine_content,
            "loss_model": "self.enhanced_loss_model.update(" in engine_content
        }
        
        # Check legacy systems still used
        legacy_usage = {
            "legacy_drivetrain": "self.drivetrain.update(" in engine_content or "self.drivetrain.apply_torque(" in engine_content,
            "legacy_generator": "self.generator.calculate_power_output(" in engine_content or "self.generator.get_load_torque(" in engine_content
        }
    ...
    if app_file.exists():
        ...
        endpoint_check = {
            ...
            "grid_services_status": "@app.route(\"/data/grid_services_status\")" in app_content,
            "control_system_status": "@app.route(\"/data/control_system_status\")" in app_content,
            "loss_analysis": "@app.route(\"/data/loss_analysis\")" in app_content,
            "pneumatic_status": "@app.route(\"/data/pneumatic_status\")" in app_content  # This should exist
        }
    ...
    if index_file.exists():
        ...
        ui_sections = {
            "pneumatic_system": "Phase 7 Pneumatic System" in template_content,  # Should exist
    ...
    existing_files = sum(len(status["files_exist"]) for status in integration_status.values())
    total_size = sum(status["total_size"] for status in integration_status.values())
    
    print(f"Advanced Systems Coverage: {existing_files}/{total_advanced_files} files ({existing_files/total_advanced_files*100:.1f}%)")
    ...
    if existing_files > 0:
        ...
        for system_name, status in integration_status.items():
            if len(status["files_exist"]) > 0:
                unused_systems.append(f"- {system_name} ({len(status['files_exist'])} files, {status['total_size']:,} bytes)")
