...
class TestDrivetrainLosses:
    ...
    def test_comprehensive_loss_calculation(self):
        ...
        total_losses = losses.calculate_total_losses({'test_component': component_state})
...
class TestElectricalLosses:
    ...
    def test_copper_losses(self):
        ...
        copper_loss = losses.calculate_copper_losses(electrical_state['current'], electrical_state['temperature'])
        ...
        high_copper_loss = losses.calculate_copper_losses(high_current_state['current'], high_current_state['temperature'])
    ...
    def test_iron_losses(self):
        ...
        iron_loss = losses.calculate_iron_losses(electrical_state['frequency'], electrical_state['flux_density'], electrical_state['temperature'])
        assert iron_loss > 0
        
        # Test frequency dependence
        high_freq_state = electrical_state.copy()
        high_freq_state['frequency'] = 120.0
        high_iron_loss = losses.calculate_iron_losses(high_freq_state['frequency'], high_freq_state['flux_density'], high_freq_state['temperature'])
    ...
    def test_switching_losses(self):
        ...
        switching_loss = losses.calculate_switching_losses(electrical_state['switching_frequency'], electrical_state['voltage'], electrical_state['current'])
        ...
        high_switching_loss = losses.calculate_switching_losses(high_switch_state['switching_frequency'], high_switch_state['voltage'], high_switch_state['current'])
...
class TestThermalModel:
    ...
    def test_thermal_model_initialization(self):
        ...
        assert len(thermal.component_states) >= 0
    ...
    def test_heat_generation_calculation(self):
        ...
        heat_generated = thermal.calculate_heat_generation('test_component', 3500.0)
        assert heat_generated > 0
        assert heat_generated == 3500.0
    ...
    def test_heat_transfer_calculation(self):
        ...
        # Add component to test heat transfer
        thermal.add_component('test_component', thermal_state)
        heat_transfer = thermal.calculate_convective_heat_transfer(thermal_state)
    ...
    def test_temperature_update(self):
        ...
        from simulation.physics.thermal import ThermalState
        thermal_state = ThermalState(temperature=20.0)
        thermal.add_component(component_name, thermal_state)
        initial_temp = thermal.component_states[component_name].temperature
        
        # Add significant heat generation
        heat_generation = 5000.0  # 5kW
        dt = 1.0  # 1 second
        
        # Set heat generation and update temperature
        thermal.calculate_heat_generation(component_name, heat_generation)
        thermal.update_component_temperature(component_name, dt)
        
        final_temp = thermal.component_states[component_name].temperature
    ...
    def test_temperature_effects_on_efficiency(self):
        ...
        # Add a generator component and test temperature effects on efficiency
        from simulation.physics.thermal import ThermalState
        generator_state_20c = ThermalState(temperature=20.0)
        generator_state_80c = ThermalState(temperature=80.0)
        generator_state_120c = ThermalState(temperature=120.0)
        
        efficiency_20c = thermal.calculate_temperature_effects_on_efficiency('generator', 0.95)
        thermal.add_component('generator', generator_state_20c)
        efficiency_20c = thermal.calculate_temperature_effects_on_efficiency('generator', 0.95)
        
        thermal.component_states['generator'] = generator_state_80c
        efficiency_80c = thermal.calculate_temperature_effects_on_efficiency('generator', 0.95)
        
        thermal.component_states['generator'] = generator_state_120c
        efficiency_120c = thermal.calculate_temperature_effects_on_efficiency('generator', 0.95)
