...
class TestPriceForecaster:
    ...
    def test_forecast_accuracy(self):
        ...
        if len(self.forecaster.forecast_accuracy) > 0:
            assert all(acc >= 0.0 for acc in self.forecaster.forecast_accuracy)
...
class TestIntegrationScenarios:
    ...
    def test_economic_optimization_integration(self):
        ...
        if 'service_allocation' in optimization_result:
            # Handle case where service_allocation values are floats (allocated capacities)
            total_allocated = sum(
                allocation if isinstance(allocation, (int, float)) else allocation.get('capacity', 0.0)
