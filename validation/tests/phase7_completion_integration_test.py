...
import time
import logging
from simulation.pneumatics.energy_analysis import EnergyAnalyzer, create_standard_energy_analyzer, EnergyFlowType
from simulation.pneumatics.performance_metrics import PerformanceAnalyzer, create_standard_performance_analyzer
from simulation.pneumatics.performance_metrics import OptimizationTarget
...
def test_complete_energy_analysis():
    ...
    analyzer.record_power_metrics(
        compressor_power=4200.0,
        mechanical_power=2500.0,
        heat_loss_rate=100.0
    )
    ...
    print(f"  Conservation Error: {validation_result.get('conservation_error_percent', 0):.2f}%")
...
def test_performance_optimization_workflow():
    ...
    efficiency_improvement = (optimized_summary.get('average_efficiency', 0) - 
...
def test_real_world_simulation():
    ...
    for sample in range(total_samples):
        ...
        efficiency_variation = 0.15 * (0.5 - abs(hour_of_day/24 - 0.5))  # Best at midday
    ...
    print(f"  Average Efficiency: {performance_summary.get('average_efficiency', 0):.3f}")
    ...
    print(f"  Total Energy Output: {energy_summary.get('total_output_energy', 0):.1f} J")
    print(f"  Overall System Efficiency: {energy_summary.get('overall_efficiency', 0):.3f}")
    
    print(f"\nTrend Analysis:")
    print(f"  Efficiency Trend: {trend_analysis.get('efficiency_trend', 'N/A')}")
    print(f"  Power Trend: {trend_analysis.get('power_trend', 'N/A')}")
    print(f"  Performance Stability: {trend_analysis.get('performance_stability', 'N/A')}")
...
def main():
    ...
    try:
        ...
        baseline_perf, optimized_perf, recommendations = test_performance_optimization_workflow()
        ...
        print(f"\nKey Results:")
        print(f"  ✓ Energy conservation validation: {validation_result.get('conservation_valid', False)}")
        print(f"  ✓ Efficiency optimization: {len(recommendations)} recommendations generated")
        print(f"  ✓ Performance improvement: {(optimized_perf.get('average_efficiency', 0) - baseline_perf.get('average_efficiency', 0)):.3f}")
        print(f"  ✓ 24-hour simulation: {daily_performance.get('average_efficiency', 0):.3f} average efficiency")
        print(f"  ✓ System availability: {daily_performance.get('availability', 0):.3f}")
        ...
        success_criteria = [
            ...
            optimized_perf.get('average_efficiency', 0) > baseline_perf.get('average_efficiency', 0),
