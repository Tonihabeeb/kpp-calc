...
def test_complete_integration():
    ...
    compression_work = pneumatic_system.calculate_compression_work(
        initial_pressure=101325.0, final_pressure=250000.0, volume=0.01
    )
    ...
    result = pneumatic_system.inject_air(
        target_depth=10.0, water_pressure=200000.0, duration=2.0
    )
    print(
        f"   ✓ Air injection: {result['volume_injected']:.4f} m³ at {result['pressure']/100000:.2f} bar"
    )
    ...
    buoyancy_data = pneumatic_system.calculate_buoyancy_change(
        air_volume=0.02, depth=10.0, water_temperature=288.15
    )
    ...
    # Test venting system
    vent_result = pneumatic_system.vent_air(vent_duration=1.0, target_pressure=150000.0)
    ...
    efficiency = coordinator.calculate_thermal_efficiency(
        coordinator.sensors.compressor_temp.value, coordinator.sensors.water_temp.value
    )
...
def test_phase_compatibility():
    ...
    for i, config in enumerate(configs, 1):
        ...
        print(
            f"   ✓ Pneumatic system thermodynamics: {pneumatic_system.enable_thermodynamics}"
        )
