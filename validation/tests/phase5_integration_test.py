...
def test_enhanced_pneumatic_system():
    ...
    floater = Floater(
        mass=10.0, volume=0.012, area=0.5
    )  # 10kg, 12L floater, 0.5m² area
    ...
    cycle_analysis = pneumatics.get_thermodynamic_cycle_analysis(
        getattr(floater, "air_volume", 0.006), 20.0
    )  # 20 second ascent

    if cycle_analysis:
        print(f"✓ Thermodynamic cycle analysis completed")
        print(
            f"  Energy balance keys: {list(cycle_analysis.get('energy_balance', {}).keys())}"
        )
        if "performance_metrics" in cycle_analysis:
            perf = cycle_analysis["performance_metrics"]
            print(
                f"  Power enhancement: {perf.get('power_enhancement_factor', 1.0):.2f}"
            )
...
def test_simulation_integration():
    ...
    try:
        ...
        if hasattr(sim, "pneumatics") and hasattr(
            sim.pneumatics, "enable_thermodynamics"
        ):
            if sim.pneumatics.enable_thermodynamics:
                print(f"✓ Phase 5 thermodynamics integrated into simulation")
                print(
                    f"  Advanced thermodynamics: {hasattr(sim.pneumatics, 'advanced_thermo')}"
                )
                print(f"  Heat exchange: {hasattr(sim.pneumatics, 'heat_exchange')}")
                print(
                    f"  Thermal buoyancy: {hasattr(sim.pneumatics, 'thermal_buoyancy')}"
                )
            else:
...
def test_performance_comparison():
    ...
    pneumatics_basic = PneumaticSystem(tank_pressure=2.5, enable_thermodynamics=False)
    ...
    basic_analysis = pneumatics_basic.get_thermodynamic_cycle_analysis(
        getattr(floater, "air_volume", 0.006), 15.0
    )
    ...
    advanced_analysis = pneumatics_advanced.get_thermodynamic_cycle_analysis(
        getattr(floater, "air_volume", 0.006), 15.0
    )
    ...
    print(
        f"  Improvement: {advanced_boost - basic_boost:.2f} N ({((advanced_boost - basic_boost) / max(basic_boost, 0.1)) * 100:.1f}%)"
    )

    print(
        f"  Basic system analysis: {'Available' if basic_analysis else 'Not available'}"
    )
    print(
        f"  Advanced system analysis: {'Available' if advanced_analysis else 'Not available'}"
    )
...
def main():
    ...
    try:
        ...
        if integration_success:
            ...
            print(
                "Phase 5 components are functional but may need additional integration work"
            )
