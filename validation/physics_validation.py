...
class ValidationFramework:
    ...
    def validate_energy_conservation(self, energy_in: float, energy_out: float, 
        ...
        relative_error = conservation_error / max(energy_in, 1e-6)  # Avoid division by zero
        ...
        if is_valid:
            ...
            self.logger.info(f"Energy conservation PASSED: {relative_error:.4f} < {self.energy_tolerance}")
        else:
            self.performance_metrics['failed_tests'] += 1
            self.logger.warning(f"Energy conservation FAILED: {relative_error:.4f} >= {self.energy_tolerance}")
        
        return result
    
    def validate_force_balance(self, forces: List[float], expected_net: float = 0.0) -> Dict[str, Any]:
        """
        ...
        if is_valid:
            self.performance_metrics['passed_tests'] += 1
            self.logger.info(f"Force balance PASSED: error {force_error:.6f} N < {self.force_tolerance} N")
        else:
            self.performance_metrics['failed_tests'] += 1
            self.logger.warning(f"Force balance FAILED: error {force_error:.6f} N >= {self.force_tolerance} N")
        
        return result
    
    def validate_physics_consistency(self, floaters: List, chain_velocity: float, 
        ...
        for i, floater in enumerate(floaters):
            ...
            if floater.state == "heavy" and abs(floater.mass - expected_heavy_mass) > 0.1:
                consistency_checks['mass_state_consistency'] = False
                issues.append(f"Floater {i}: Heavy state but incorrect mass")
            elif floater.state == "light" and abs(floater.mass - floater.container_mass) > 0.1:
                consistency_checks['mass_state_consistency'] = False
                issues.append(f"Floater {i}: Light state but incorrect mass")
            
            # Check velocity consistency with chain
            expected_velocity = chain_velocity if floater.angle < math.pi else -chain_velocity
            velocity_error = abs(floater.velocity - expected_velocity) / max(abs(chain_velocity), 0.1)
            ...
            if hasattr(floater, 'last_buoyant_force'):
                ...
                buoyancy_error = abs(floater.last_buoyant_force - expected_buoyancy) / expected_buoyancy
        ...
        if is_valid:
            ...
            self.performance_metrics['failed_tests'] += 1
            self.logger.warning(f"Physics consistency FAILED: {len(issues)} issues found")
            for issue in issues:
    ...
    def run_comprehensive_validation(self, simulation_engine) -> Dict[str, Any]:
        ...
        # Generate summary
        validation_session['summary'] = self._generate_validation_summary(validation_session['tests'])
        validation_session['recommendations'] = self._generate_recommendations(validation_session['tests'])
    ...
    def _test_single_floater_physics(self, simulation_engine) -> Dict[str, Any]:
        ...
        try:
            ...
            single_floater.mass = single_floater.container_mass + 1000 * single_floater.volume
            ...
            forces_history = []
            for _ in range(10):
                forces = simulation_engine.physics_engine.calculate_forces([single_floater], 1.0)
                forces_history.append(forces[0])
                
                # Basic physics update
                net_force = sum([forces[0]['buoyant'], forces[0]['gravitational'], forces[0]['drag']])
                acceleration = net_force / single_floater.mass
                single_floater.velocity += acceleration * simulation_engine.physics_engine.params['time_step']
            
            # Check force consistency
            force_magnitudes = [abs(f['buoyant']) for f in forces_history]
            buoyancy_consistent = all(abs(f - force_magnitudes[0]) < 1.0 for f in force_magnitudes)
        ...
        return {'test_name': 'single_floater_physics', 'passed': False, 'notes': 'Test failed with exception'}
    ...
    def _test_multi_floater_system(self, simulation_engine) -> Dict[str, Any]:
        ...
        try:
            ...
            total_force_before = sum([
                simulation_engine.physics_engine.calculate_forces(simulation_engine.floaters, 1.0)[i]['total']
                for i in range(len(simulation_engine.floaters))
            ])
            
            # Check that forces are reasonable
            reasonable_forces = abs(total_force_before) < 10000  # Reasonable force magnitude
    ...
    def _test_energy_balance_over_time(self, simulation_engine) -> Dict[str, Any]:
        ...
        try:
            ...
            initial_kinetic = sum([0.5 * f.mass * f.velocity**2 for f in simulation_engine.floaters])
            initial_potential = sum([f.mass * 9.81 * f.angle for f in simulation_engine.floaters])  # Simplified
            ...
            for _ in range(5):
                ...
                current_kinetic = sum([0.5 * f.mass * f.velocity**2 for f in simulation_engine.floaters])
                current_potential = sum([f.mass * 9.81 * f.angle for f in simulation_engine.floaters])
                current_total = current_kinetic + current_potential
                energy_history.append(current_total)
            
            # Check energy drift
            energy_drift = abs(energy_history[-1] - energy_history[0]) / max(energy_history[0], 1e-6)
    ...
    def _test_equilibrium_behavior(self, simulation_engine) -> Dict[str, Any]:
        ...
        try:
            ...
            forces = simulation_engine.physics_engine.calculate_forces(simulation_engine.floaters, 0.1)
        ...
        for test in tests:
            if not test.get('passed', False):
                ...
                if 'energy' in test_name:
                    recommendations.append("Consider reviewing energy calculation methods and conservation logic")
                elif 'force' in test_name:
                    recommendations.append("Check force calculation accuracy and balance")
                elif 'equilibrium' in test_name:
                    recommendations.append("Review equilibrium conditions and force balance at rest")
                else:
                    recommendations.append(f"Investigate issues in {test_name} test")
        
        if not recommendations:
            recommendations.append("All validation tests passed - system appears to be functioning correctly")
    ...
    def get_performance_metrics(self) -> Dict[str, Any]:
        ...
        total_tests = self.performance_metrics['passed_tests'] + self.performance_metrics['failed_tests']
        
        return {
            **self.performance_metrics,
            'total_tests': total_tests,
            'success_rate': self.performance_metrics['passed_tests'] / max(total_tests, 1)
        }
