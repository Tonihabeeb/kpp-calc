...
print(
    f" ✅ Complete electromechanical system: {all([has_drivetrain, has_electrical, has_control, has_loss_model])}"
)
print(f" ✅ Enhanced loss model operational: {engine.enhanced_loss_model is not None}")
print(
    f" ✅ Thermal tracking active: {len(engine.enhanced_loss_model.thermal_model.component_states)} components"
)
...
print(
    f' ✅ Component temperatures: {[f"{name}:{state.temperature:.1f}°C" for name, state in thermal_states.items()]}'
)
print()
