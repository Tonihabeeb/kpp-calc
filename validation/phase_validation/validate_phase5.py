...
print(f'    Ambient temperature: {engine.enhanced_loss_model.thermal_model.ambient_temperature}°C')
print(f'    Thermal components: {len(engine.enhanced_loss_model.thermal_model.component_states)}')
print()
...
print(f'    Model created with ambient: {loss_model.thermal_model.ambient_temperature}°C')
...
initial_temp = engine.enhanced_loss_model.thermal_model.component_states['gearbox'].temperature
...
after_steps_temp = engine.enhanced_loss_model.thermal_model.component_states['gearbox'].temperature
print(f'    Temperature after steps: {after_steps_temp:.2f}°C')

# Reset and verify
engine.reset()
reset_temp = engine.enhanced_loss_model.thermal_model.component_states['gearbox'].temperature
...
system_state = {
    'drivetrain': {
        'gearbox': {'torque': 1200.0, 'speed': 120.0, 'temperature': 30.0, 'load_factor': 0.5}
    },
    'generator': {
        'torque': 1150.0,
        'speed': 250.0,
...
print(f'    Total system losses: {enhanced_state.system_losses.total_system_losses:.2f}W')
print(f'    Mechanical losses: {enhanced_state.system_losses.mechanical_losses.total_losses:.2f}W')
...
print(f'    Average temperature: {enhanced_state.performance_metrics["average_temperature"]:.2f}°C')
print()
...
gearbox_20c = loss_model.thermal_model.calculate_temperature_effects_on_efficiency('gearbox', base_efficiency=0.95)
loss_model.thermal_model.component_states['gearbox'].temperature = 60.0
gearbox_60c = loss_model.thermal_model.calculate_temperature_effects_on_efficiency('gearbox', base_efficiency=0.95)
loss_model.thermal_model.component_states['gearbox'].temperature = 100.0
gearbox_100c = loss_model.thermal_model.calculate_temperature_effects_on_efficiency('gearbox', base_efficiency=0.95)
...
print(f'    Temperature degradation working: {gearbox_100c < gearbox_60c < gearbox_20c}')
print()
