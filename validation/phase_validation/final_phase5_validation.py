...
def main():
    ...
    print(
        "Testing enhanced loss model integration..."
    )  # Create engine and verify enhanced loss model is initialized
    data_queue = queue.Queue()
    engine = SimulationEngine(params, data_queue)
    print(
        f" ✅ Enhanced loss model initialized: {engine.enhanced_loss_model is not None}"
    )
    print(
        f" ✅ Thermal model initialized: {engine.enhanced_loss_model.thermal_model is not None}"
    )
    ...
    thermal_components = list(
        engine.enhanced_loss_model.thermal_model.component_states.keys()
    )
    print(f"   Thermal components tracked: {len(thermal_components)}")
    for comp in thermal_components:
        temp = engine.enhanced_loss_model.thermal_model.component_states[
            comp
        ].temperature
