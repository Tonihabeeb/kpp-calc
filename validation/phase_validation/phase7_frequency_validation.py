...
def test_primary_frequency_control():
    ...
    print(f"{'Frequency (Hz)':<15} {'Power Command (MW)':<20} {'Status':<30} {'Description'}")
    print("-" * 95)
    
    for freq, description in scenarios:
        response = controller.update(freq, 0.1, rated_power)
        print(f"{freq:<15.2f} {response['power_command_mw']:<20.2f} {response['status']:<30} {description}")
...
def test_secondary_frequency_control():
    ...
    print(f"{'AGC Signal':<12} {'Power Command (MW)':<20} {'Status':<25} {'Description'}")
    print("-" * 85)
    ...
    for agc_signal, description in scenarios:
        ...
        if response is not None:
            print(f"{agc_signal:<12.2f} {response['power_command_mw']:<20.2f} {response['status']:<25} {description}")
        else:
            print(f"{agc_signal:<12.2f} {'ERROR':<20} {'Controller returned None':<25} {description}")
        controller.reset()  # Reset for next test
...
def test_synthetic_inertia():
    ...
    print(f"{'Time (s)':<10} {'Frequency (Hz)':<15} {'ROCOF (Hz/s)':<15} {'Power (MW)':<15} {'Active':<10}")
    print("-" * 75)
    
    for i, freq in enumerate(frequencies):
        response = controller.update(freq, 0.1, rated_power)
        print(f"{i*0.1:<10.1f} {freq:<15.2f} {response['rocof_hz_per_s']:<15.3f} "
    ...
    for i in range(20):
        ...
        if i % 5 == 0:  # Print every 5th update
            print(f"{(len(frequencies) + i)*0.1:<10.1f} {59.5:<15.2f} {response['rocof_hz_per_s']:<15.3f} "
                  f"{response['power_command_mw']:<15.2f} {controller.is_responding()}")
...
def test_grid_services_coordination():
    ...
    for _ in range(10):
        response = coordinator.update(grid_conditions, 0.1, rated_power_mw)
        grid_conditions.frequency = max(59.80, grid_conditions.frequency - 0.01)  # Gradual decline
...
def test_integration_scenario():
    ...
    print(f"{'Time':<6} {'Freq':<6} {'AGC':<6} {'Power Cmd':<12} {'Services':<8} {'Description'}")
    print("-" * 80)
    ...
    for time_step, freq, agc, description in scenarios:
        ...
        print(f"{time_step:<6.1f} {freq:<6.2f} {agc:<6.2f} {response['total_power_command_mw']:<12.2f} "
...
def main():
    ...
    try:
        ...
        print("✅ Phase 7 Week 1 implementation is functional and ready for integration")
