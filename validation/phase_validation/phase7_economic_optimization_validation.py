...
from simulation.grid_services.economic.economic_optimizer import create_economic_optimizer
from simulation.grid_services.economic.market_interface import create_market_interface, MarketType
from simulation.grid_services.economic.bidding_strategy import (
    create_bidding_strategy, MarketConditions, OperationalConstraints
)

# Import grid services coordinator
from simulation.grid_services.grid_services_coordinator import (
    GridServicesCoordinator, GridConditions, GridServicesConfig
)
...
class EconomicOptimizationValidator:
    ...
    def __init__(self):
        ...
        # Initialize services
        self.price_forecaster = create_price_forecaster(base_price=60.0, volatility=0.20)
        self.economic_optimizer = create_economic_optimizer(max_power_kw=250.0, risk_tolerance=0.3)
    ...
    def validate_price_forecasting(self):
        ...
        try:
            ...
            assert all(isinstance(p, float) and p > 0 for p in result['forecast_prices'])
        ...
        try:
            ...
            accuracy_count = len(self.price_forecaster.forecast_accuracy) if hasattr(self.price_forecaster, 'forecast_accuracy') else 0
    ...
    def validate_economic_optimization(self):
        ...
        try:
            ...
            risky_services = {
                'high_value_service': {'capacity': 100.0, 'price': 200.0}
            }
            
            forecasts = {'prices': [150.0] * 24}
            
            result = self.economic_optimizer.optimize(high_risk_conditions, risky_services, forecasts)
    ...
    def validate_market_interface(self):
        ...
        try:
            ...
            # Check bid tracking
            total_bids = len(self.market_interface.active_bids) + len(self.market_interface.bid_history)
            assert total_bids >= 0
        ...
        try:
            ...
            clearings = self.market_interface.market_clearings[MarketType.ENERGY_REAL_TIME]
    ...
    def validate_bidding_strategy(self):
        ...
        try:
            ...
            for rec in recommendations:
                ...
                assert rec.service_type in ['energy', 'frequency_regulation', 'spinning_reserve']
            
            print(f"✅ Bid recommendations (generated {len(recommendations)} recommendations)")
            self.passed_count += 1
        ...
        try:
            ...
            expected_keys = ['suggested_strategy', 'risk_adjustment', 'markup_range', 'volume_guidance']
    ...
    def validate_integration_scenario(self):
        ...
        try:
            ...
            print(f"✅ Bidding strategy to market interface integration ({successful_submissions} successful submissions)")
            self.passed_count += 1
    ...
    def validate_coordinator_integration(self):
        ...
        try:
            ...
            economic_services = ['economic_optimizer', 'market_interface', 'price_forecaster', 'bidding_strategy']
