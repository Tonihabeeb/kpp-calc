...
def simulate_system_startup():
    ...
    for step in range(10):
        ...
        system_state = {
            ...
            "floaters": [
                {"id": 0, "fill_progress": min(1.0, step * 0.2)},
                {"id": 1, "fill_progress": min(1.0, step * 0.15)},
            ],
        ...
        commands = startup_controller.update_startup_sequence(
        ...
        if commands.get("startup_active", False):
            ...
            print(
                f"   t={current_time:4.1f}s: Phase '{phase}' - Progress: {progress:.1%}"
            )
...
def simulate_emergency_response():
    ...
    emergency_scenarios = [
        {
            "name": "Overspeed",
            "flywheel_speed_rpm": 500.0,
            "expected": "Critical emergency shutdown",
        },
        {
            "name": "Overpressure",
            "pneumatics": {"tank_pressure": 12.0},
            "expected": "Critical emergency shutdown",
        },
        {
            "name": "Overtemperature",
            "component_temperatures": {"generator": 95.0},
            "expected": "High priority response",
        },
    ]
    ...
    for i, scenario in enumerate(emergency_scenarios):
        ...
        system_state = {
            ...
            "component_temperatures": scenario.get(
        ...
        commands = emergency_system.monitor_emergency_conditions(
            system_state, current_time
        )
        ...
        if commands.get("emergency_active", False):
            ...
            print(
                f"   Actual response: Emergency active = {commands.get('emergency_active', False)}"
            )
            ...
            if active:
                ...
                active_names = [
                    str(em.value) if hasattr(em, "value") else str(em) for em in active
                ]
...
def simulate_grid_disturbances():
    ...
    disturbance_scenarios = [
        {
            "name": "Voltage Drop",
            "grid_voltage": 430.0,
            "expected": "Voltage support response",
        },
        {
            "name": "Frequency Drop",
            "grid_frequency": 59.4,
            "expected": "Frequency support response",
        },
        {
            "name": "Grid Outage",
            "grid_connected": False,
            "expected": "Disconnect and load shedding",
        },
        {
            "name": "Overvoltage",
            "grid_voltage": 520.0,
            "expected": "Voltage regulation response",
        },
    ]
    ...
    for i, scenario in enumerate(disturbance_scenarios):
        ...
        if commands.get("grid_disturbance_active", False):
            ...
            print(
                f"   Disconnect recommended: {commands.get('disconnect_required', False)}"
            )
            ...
            print(
                f"   Disturbance type: {disturbance_type}, magnitude: {magnitude:.3f}"
            )
        else:
...
def simulate_coordinated_response():
    ...
    for i, scenario in enumerate(coordination_scenarios):
        ...
        commands = transient_controller.update_transient_events(
            system_state, current_time
        )
...
def main():
    ...
    print(
        "Testing startup management, emergency response, grid disturbance handling, and coordination"
    )
    ...
    try:
        ...
        print(
            "   The system can safely manage startup sequences, respond to emergencies,"
        )
        print(
            "   handle grid disturbances, and coordinate multiple simultaneous events."
        )
