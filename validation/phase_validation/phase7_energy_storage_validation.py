...
def validate_battery_storage_system():
    ...
    for scenario in scenarios:
        ...
        if BatteryMode(status["mode"]) == scenario["expected_mode"]:
            ...
            print(
                f"  ✗ FAILED: Expected {scenario['expected_mode'].value}, got {status['mode']}"
            )
...
def validate_grid_stabilization_controller():
    ...
    for scenario in stabilization_scenarios:
        ...
        if "control_commands" in result:
            ...
            print(
                f"  Reactive Power: {commands.get('reactive_power_kvar', 0):.1f} kVAR"
            )
        ...
        if StabilizationMode(result["mode"]) == scenario["expected_mode"]:
            ...
            print(
                f"  ✗ FAILED: Expected {scenario['expected_mode'].value}, got {result['mode']}"
            )
...
def validate_grid_services_integration():
    ...
    for scenario in integration_scenarios:
        ...
        response = coordinator.update(
            grid_conditions=scenario["conditions"], dt=1.0, rated_power=200.0  # MW
        )
        ...
        print(
            f"  Total Power Command: {response.get('total_power_command_mw', 0.0):.2f} MW"
        )
        print(f"  Active Services: {len(response.get('active_services', []))}")
        print(f"  Service Types: {', '.join(response.get('active_services', []))}")
        print(
            f"  Coordination Method: {response.get('coordination_method', 'unknown')}"
        )
        ...
        print(
            f"  Grid Stabilization: {service_status.get('grid_stabilization', False)}"
        )
        print(
            f"  Frequency Control: {service_status.get('primary_frequency_control', False)}"
        )
...
def validate_economic_performance():
    ...
    for hour, price in enumerate(hourly_prices):
        ...
        if hour % 6 == 0:  # Every 6 hours
            print(
                f"Hour {hour:2d}: Price ${price:3.0f}/MWh, "
    ...
    print(
        f"Round-trip Efficiency: {final_status['total_energy_discharged']/max(final_status['total_energy_charged'], 1)*100:.1f}%"
    )
    print(f"Arbitrage Revenue: ${final_status['arbitrage_revenue']:.2f}")
    print(f"Grid Service Revenue: ${final_status['grid_service_revenue']:.2f}")
    print(f"Total Revenue: ${final_status['total_revenue']:.2f}")
    print(
        f"Revenue per kWh Capacity: ${final_status['total_revenue']/battery.specs.nominal_capacity_kwh:.2f}/kWh"
    )
...
def main():
    ...
    try:
        ...
        if passed == total:
            ...
            print(
                "Phase 7 Week 4 Energy Storage Integration is fully implemented and validated."
            )
        else:
