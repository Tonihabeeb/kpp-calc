...
def validate_load_curtailment():
    ...
    print(f"   Normal conditions - Curtailment active: {response['curtailment_active']}")
    print(f"   Curtailment amount: {response['curtailment_amount']:.2f} MW")
    assert not response['curtailment_active'], "Should not curtail under normal conditions"
    ...
    print(f"   Emergency conditions - Curtailment active: {response['curtailment_active']}")
    print(f"   Curtailment amount: {response['curtailment_amount']:.2f} MW")
    ...
    if controller.active_event:
        print(f"   Requested reduction: {controller.active_event.requested_reduction:.2f} MW")
    ...
    if response['curtailment_amount'] == 0:
        ...
        assert response['curtailment_amount'] > 0, "Should have positive curtailment amount"
    ...
    print(f"   High price conditions - Curtailment active: {response['curtailment_active']}")
    print(f"   Curtailment amount: {response['curtailment_amount']:.2f} MW")
    if response.get('active_event_type'):
        print(f"   Curtailment type: {response['active_event_type']}")
...
def validate_peak_shaving():
    ...
    response = controller.update(9.5, 4.0, 1.0)  # current_demand=9.5MW (above 9MW threshold)
...
def validate_load_forecaster():
    ...
    if forecast:
        for i, point in enumerate(forecast[:3]):  # Show first 3 points
            print(f"   Hour {i+1}: {point['predicted_load']:.2f} MW (confidence: {point['confidence']:.2f})")
