    ...
    async def connect_to_master_clock(self):
        ...
        try:
            self.master_clock_connection = await websockets.connect(
                f"{self.master_clock_url}?type=websocket_server",
                ping_interval=10,
                ping_timeout=5
            )
            ...
            if status_response.status_code == 200:
                ...
                if simulation_running:
                    ...
                    try:
                        ...
                        try:
                            summary_response = requests.get(f"{self.backend_url}/data/summary", timeout=2, headers=headers)
                            if summary_response.status_code == 200:
                                ...
                                self.last_good_data = kpp_data
                                logging.debug(f"Fetched summary data with trace {trace_id}: power={kpp_data['power']:.2f}W")
                                return kpp_data
                        except:
                            pass
                    except Exception as e:
                ...
                if simulation_running:
                    ...
                    logging.debug(f"Using cached running data with trace {trace_id}: power={running_data['power']:.2f}W")
                    return running_data
        ...
        def loop():
            while self.running:
                ...
                if self.tick % 25 == 0:  # Log every 5 seconds at 5Hz
                    logging.info(f"WebSocket tick {self.tick}: {active_clients} active clients, power={data.get('power', 0):.2f}W")
...
@app.get("/state")
def get_state():
    """Get current KPP simulation state - endpoint for Dash app"""
    trace_id = f"{ws_core.session_trace_id}-state-{int(time.time() * 1000)}"
    
    try:
        data = ws_core.fetcher.get_kpp_data()
        logging.debug(f"State request with trace {trace_id}: power={data.get('power', 0):.2f}W")
        return {
            "status": "success",
            "timestamp": time.time(),
            "trace_id": trace_id,
        ...
        logging.error(f"Error getting state with trace {trace_id}: {e}")
        return {
            "status": "error",
            "message": str(e),
            "timestamp": time.time(),
            "trace_id": trace_id,
            "simulation_data": ws_core.fetcher.last_good_data
        }
