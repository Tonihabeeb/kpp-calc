...
class MasterClockServer:
    ...
    def fetch_simulation_data(self) -> FrameData:
        """Fetch latest simulation data from backend with timeout protection"""
        try:
            # Use shorter timeout to prevent hanging
            status_response = requests.get(f"{self.backend_url}/status", timeout=0.2)
            status_data = status_response.json() if status_response.status_code == 200 else {}
            
            # Get live data with timeout
            live_response = requests.get(f"{self.backend_url}/data/live", timeout=0.2)
            ...
            return FrameData(
                ...
                efficiency=0.0,
                status='timeout',
                metadata={'error': 'Backend timeout'}
            )
    ...
    async def sync_loop(self):
        ...
        while self.running:
            try:
                ...
                # Fetch latest simulation data (with timeout protection)
...
if __name__ == "__main__":
    logger.info("Starting KPP Master Clock Server on port 9201")
    uvicorn.run(app, host="0.0.0.0", port=9201, log_level="info") 